...
def _field(n: Node, name: str) -> Optional[Node]:
...
def _kids(n: Node, type_: str) -> list:
    return [c for c in n.named_children if c.type == type_]


def _kid(n: Node, type_: str) -> Optional[Node]:
...
def _parse_source_uncached(src: str) -> dict:
    ...
    for c in tree.root_node.named_children:
        ...
        if d is not None:
            ...
            start = prev.start_point if exported else c.start_point
            d["loc"] = {"sl": start[0], "sc": start[1], "el": c.end_point[0], "ec": c.end_point[1]}
            decls.append(d)
    return {"decls": decls, "errors": errors}


def _collect_errors(n: Node, out: list) -> None:
...
# ---------------- declarations ----------------
def lower_decl(n: Node) -> Optional[dict]:
    t = n.type
    if t == "type_declaration":
        params = _kid(n, "type_parameters")
        return {"d": "type", "name": _text(_req(n, "name")),
                "params": [{"name": _text(p.named_children[0]),
                            "type": lower_type(p.named_children[1]) if len(p.named_children) > 1 else None}
                           for p in _kids(params, "type_parameter")] if params else None,
                "type": lower_type(_req(n, "type")), "tail": _maybe_tail(n)}
    if t == "const_declaration":
        return {"d": "const", "name": _text(_req(n, "name")),
                "type": lower_type(_req(n, "type")) if _field(n, "type") else None,
                "expr": lower_expr(_req(n, "value"))}
    if t == "func_declaration":
        return {"d": "func", "name": _text(_req(n, "name")),
                "params": [{"name": _text(p.named_children[0]), "type": lower_type(p.named_children[1])}
    ...
    if t == "output_declaration":
        return {"d": "output", "name": _text(_req(n, "name")), "type": lower_type(_req(n, "type")),
                "expr": lower_expr(_req(n, "value"))}
    if t == "input_declaration":
        return {"d": "input", "name": _text(_req(n, "name")), "type": lower_type(_req(n, "type")),
                "fallback": lower_expr(_req(n, "fallback")) if _field(n, "fallback") else None}
    if t == "diagnostic_declaration":
        sev = _kid(n, "severity")
        tmpl = _kid(n, "template_string")
        return {"d": "diagnostic", "name": _text(_req(n, "name")),
                "params": [{"name": _text(p.named_children[0]), "type": lower_type(p.named_children[1])}
                           for p in _kids(n, "parameter")],
                "severity": _text(sev), "template": lower_template_parts(tmpl)}
    if t == "dimension_declaration":
        e = _kid(n, "dimension_expression")
        return {"d": "dimension", "name": _text(_req(n, "name")), "terms": lower_dim_expr(e) if e else None}
    if t == "unit_declaration":
        dim = _field(n, "dimension")
        if dim is not None:
            return {"d": "unit", "name": _text(_req(n, "name")), "dim": _text(dim)}
        return {"d": "unit", "name": _text(_req(n, "name")), "factor": lower_expr(_field(n, "factor")),
                "base": _text(_field(n, "base"))}
    if t == "import_declaration":
        frm = json.loads(_text(_kid(n, "string")))
        ni = _kid(n, "named_imports")
        if ni is not None:
            return {"d": "import", "from": frm, "names": [_lower_import_item(i) for i in _kids(ni, "import_item")]}
        return {"d": "import", "from": frm, "ns": _text(_kid(n, "identifier"))}
    if t == "re_export_declaration":
        return {"d": "re_export", "from": json.loads(_text(_kid(n, "string"))),
                "names": [_lower_import_item(i) for i in _kids(n, "import_item")]}
    return None


def _lower_import_item(it: Node) -> dict:
    ids = it.named_children
    return {"name": _text(ids[0]), "as": _text(ids[1]) if len(ids) > 1 else None}


def _maybe_tail(n: Node) -> Optional[dict]:
    t = _kid(n, "else_clause")
    return _lower_tail(t) if t is not None else None


def _lower_tail(n: Node) -> dict:
    sev = _kid(n, "severity")
    if sev is not None:
        return {"t": "inline", "severity": _text(sev), "template": lower_template_parts(_kid(n, "template_string"))}
    name = _text(_kid(n, "qualified_name"))
...
def lower_template_parts(n: Node) -> list:
    parts: list = []
    for c in n.named_children:
...
# ---------------- types ----------------
def _loc_of(n: Node) -> dict:
    return {"sl": n.start_point[0], "sc": n.start_point[1], "el": n.end_point[0], "ec": n.end_point[1]}


def lower_type(n: Node) -> dict:
    t = _lower_type0(n)
    t["loc"] = _loc_of(n)
    return t


def _lower_type0(n: Node) -> dict:
    t = n.type
    if t == "union_type":
    ...
    if t == "nullable_type":
        return {"k": "union", "arms": [lower_type(n.named_children[0]), {"k": "prim", "name": "null"}]}
    if t == "array_type":
    ...
    if t == "named_type":
        ...
        if name in ("int", "uint", "float", "bool", "string") and not args and not preds and not ext:
...
def lower_dim_expr(n: Node) -> list:
    out: list = []
    sign = 1
    for c in n.children:
        if not c.is_named:
...
def _lower_member0(n: Node) -> Optional[dict]:
    t = n.type
    ...
    if t == "assert_member":
        return {"m": "assert", "name": _text(_req(n, "name")), "cond": lower_expr(_req(n, "condition")),
                "tail": _maybe_tail(n)}
    if t == "when_member":
        body: list = []
...
def lower_expr(n: Node) -> dict:
    e = _lower_expr0(n)
    e["loc"] = _loc_of(n)
    return e


def _lower_expr0(n: Node) -> dict:
    t = n.type
    if t == "int":
    ...
    if t == "object":
        ...
        for en in _kids(n, "object_entry"):
            key = _field(en, "key")
            if key is not None:
                entries.append({"key": json.loads(_text(key)) if key.type == "string" else _text(key),
    ...
    if t == "map_comprehension":
        return {"e": "mapcomp", "key": lower_expr(_req(n, "key")), "val": lower_expr(_req(n, "value")),
    ...
    if t == "array_comprehension":
        return {"e": "comp", "head": lower_expr(_req(n, "head")),
                "clauses": [_lower_for(c) for c in _kids(n, "for_clause")]}
    ...
    if t == "match_expression":
        ...
        for a in _kids(n, "match_arm"):
            ...
            arms.append({"v": _text(others[0]), "type": lower_type(others[1]) if len(others) > 1 else None,
...
def _lower_for(n: Node) -> dict:
    return {"v": _text(_req(n, "variable")), "iter": lower_expr(_req(n, "iterable")),
