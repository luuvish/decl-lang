...
def subsumes(
    env: Any, a: dict[str, Any], b: dict[str, Any], assume: dict[int, Any] | None = None
) -> bool:
    ...
    if b["t"] == "pred":
        if a["t"] == "pred":
            return subsumes(env, a["base"], b["base"], assume) and all(
                any(_pred_eq(p, q) for q in a["preds"]) for p in b["preds"]
            )
        if a["t"] == "lit":
    ...
    if bt == "arr":
        ...
        if not subsumes(env, a["elem"], b["elem"], assume):
            return False
        lo_a, hi_a, lo_b, hi_b = a.get("lo"), a.get("hi"), b.get("lo"), b.get("hi")
        a_lo, a_hi = lo_a if lo_a is not None else 0, hi_a if hi_a is not None else float("inf")
        b_lo, b_hi = lo_b if lo_b is not None else 0, hi_b if hi_b is not None else float("inf")
        try:
    ...
    if bt == "func":
        if a["t"] != "func" or len(a["params"]) != len(b["params"]):
            return False
        return all(
            subsumes(env, bp, ap, assume) for bp, ap in zip(b["params"], a["params"], strict=True)
    ...
    if bt == "rec":
        ...
        for m in b["members"]:
            ...
            def type_ok(m_types: list[Any] = m_types, s_types: list[Any] = s_types) -> bool:
                return (
                    not m_types
                    or not s_types
                    or all(any(subsumes(env, st, mt, assume) for st in s_types) for mt in m_types)
                )
            ...
            if k == "req":
                ...
                if sm is not None and not type_ok():
                    s.discard(id(b))
                    return False
            elif k == "der" and (sm is None or not type_ok()):
                s.discard(id(b))
                return False
        return True
    return bt == "any"
...
def _in_range(v: Any, r: dict[str, Any]) -> bool:
...
def _pred_eq(a: Any, b: Any) -> bool:
    ...
    if a.get("e") == "call" and b.get("e") == "call":
        return (
            ...
            and all(
                x.get("e") == "lit" and y.get("e") == "lit" and _val_eq(x["v"], y["v"])
...
def _lit_satisfies(env: Any, v: Any, pred: dict[str, Any]) -> bool:
...
def _kind_of(t: dict[str, Any]) -> str | None:
    tt = t["t"]
    if tt == "prim":
        return t["name"]
    if tt == "lit":
...
def _disjoint(env: Any, a: dict[str, Any], b: dict[str, Any]) -> bool:
