...
STD: dict[str, Any] = {
    ...
    "array.fold": (3, None),
    "array.sort": (1, None),
    "array.sort_by": (2, None),
    "array.unique": (1, None),
    "array.reverse": (1, None),
...
def infer(cx: Ctx, e: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ty = _infer0(cx, e)
        if cx.record is not None:
            cx.record(e, ty)
        if cx.resolve_hook is not None and e["e"] == "name":
            cx.resolve_hook(e, resolve_name(cx, e["name"]))
        return ty
    finally:
        cx.pos["at"] = prev
...
def resolve_in(env: Any, name: str) -> dict[str, Any] | None:
    ...
    if name in env.funcs:
        return {"kind": "func", "env": env, "name": name}
    if any(o["name"] == name for o in env.outputs):
        return {"kind": "output", "env": env, "name": name}
    ...
    if name in env.type_asts:
        return {"kind": "type", "env": env, "name": name}
    if name in env.diags:
        return {"kind": "diagnostic", "env": env, "name": name}
    return None
...
def type_text(rt: dict[str, Any] | None, seen: frozenset[int] = frozenset()) -> str:
    ...
    if t == "lit":
        return lit(rt["v"])
    if t == "range":
        return f"{lit(rt['lo'])}..{'<' if rt.get('excl') else ''}{lit(rt['hi'])}"
    if t == "pattern":
        return f"/{rt['src']}/"
    if t == "quantity":
