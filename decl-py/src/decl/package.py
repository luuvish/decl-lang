...
def parse_manifest(path: str, report: Callable[[str, str], None]) -> dict[str, Any] | None:
    try:
    ...
    for line0 in src.split("\n"):
        ...
        if section == "dependencies":
            ...
            if not isinstance(value, str) or not VERSION_RE.match(value):
                report(
                    "E3012",
                    f"manifest {path}: dependency {key} = {raw} is not an exact semantic-version "
                    f"pin",
                )
                ok = False
                continue
...
def find_package_root(from_file: str) -> str | None:
...
def open_package_universe(entry_file: str) -> dict[str, Any] | None:
    diags: list[Any] = []
    ...
    if not manifest:
        return {
            "root_dir": root_dir,
            "manifest": {"name": "?", "version": "0.0.0", "dependencies": {}},
            "packages": {},
            "resolver": lambda spec, from_dir: {"code": "E3011", "message": "unusable manifest"},
            "diags": diags,
        }
    ...
    def visit(m: dict[str, Any], by_dir: str) -> None:
        for dep, ver in m["dependencies"].items():
            prev = required_by.get(dep)
            if prev and prev["version"] != ver:
                report(
                    "E3014",
                    f"package {dep} required at {prev['version']} (by {prev['by']}) and {ver} (by "
                    f"{m['name']})",
                )
                continue
            ...
            if dm["version"] != ver:
                report(
                    "E3016",
                    f"package {dep}: manifest version {dm['version']} differs from required pin "
                    f"{ver}",
                )
            packages[dep] = {
                "name": dep,
    ...
    def resolver(spec: str, from_dir: str) -> Any:
        slash = spec.find("/")
        pkg = spec if slash < 0 else spec[:slash]
        rest = "" if slash < 0 else spec[slash + 1 :]
        # which package does the importing file belong to?
        from_pkg_dir = next(
            (p["dir"] for p in packages.values() if os.path.abspath(from_dir).startswith(p["dir"])),
            root_dir,
        )
        from_manifest = (
            manifest
            if from_pkg_dir == root_dir
            else parse_manifest(os.path.join(from_pkg_dir, "decl.toml"), lambda c, m: None)
        )
        if from_manifest is None or pkg not in from_manifest["dependencies"]:
            return {
                "code": "E3010",
                "message": f"package {pkg} not declared in [dependencies] of "
                f"{from_manifest['name'] if from_manifest else '?'}",
            }
        p = packages.get(pkg)
        if not p:
...
# ---------------- decl.lock (§8.7) ----------------
def lock_text(u: dict[str, Any]) -> str:
    lines = [
        f"{p['name']} {p['version']} {p['hash']}"
        for p in sorted(u["packages"].values(), key=lambda p: p["name"])
    ]
    return "\n".join(lines) + ("\n" if lines else "")
...
def verify_lock(u: dict[str, Any]) -> list[Any]:
    ...
    for p in u["packages"].values():
        ...
        if l["version"] != p["version"]:
            report(
                "E3016",
                f"lock: {p['name']} version {l['version']} differs from manifest {p['version']}",
            )
        elif l["hash"] != p["hash"]:
