...
from decl.runtime.semantics import ArrV, Env, Quantity, Scope, read_json
...
def pipeline(src: str, binds: list | None = None):
    ...
    for name, raw in binds or []:  # bound documents first (§9.2), then outputs
        eng.bind_root(
            name, raw, env.resolve(env.inputs[name]["type"]), Scope(None, {}, name), False
        )
    for o in env.outputs:
        eng.bind_root(
            o["name"], o["expr"], env.resolve(o["type"]), Scope(None, {}, o["name"]), True
        )
...
check(
    ...
    eng.serialize(env.roots["sweep"], "sweep") == eng.serialize(env.roots["sweep"], "sweep"),
)
...
check(
    "prod layering",
    get(eng, ["prod", "host"]) == "api.internal" and get(eng, ["prod", "workers"]) == 32,
)
check("derived recomputed after with", get(eng, ["prod", "insecure"]) is False)
env2, eng2 = pipeline(
    cfg_src,
    [("deployed", read_json('{"host":"x","port":70000,"workers":100,"tls":{"enabled":true}}'))],
)
...
check(
    "propagated master si",
    get(eng, ["xbar", "nodes", "dom0", "nodes", "mst0", "ports", "si", "width"]) == 64,
)
check(
    "arbiter max",
    get(eng, ["xbar", "nodes", "dom0", "nodes", "arb0", "ports", "mi", "width"]) == 64,
)
...
check(
    "one width_match at root cause",
    len(errs) == 1 and errs[0].get("id") == "Edge.width_match",
    str(errs),
)
...
check(
    "guide quantity default",
    isinstance(get(eng, ["demo", "services", 0, "timeout"]), Quantity)
...
check(
    "closed set resolved",
    len(u["packages"]) == 1 and u["packages"]["corelib"]["version"] == "1.0.0",
)
r = load_modules(entry, None, u["resolver"])
check(
    "modules load across packages",
    len(r["modules"]) == 2 and not r["diags"],
    json.dumps(r["diags"]),
)
...
check(
    "evaluates clean",
    not [d for d in uni["diags"] if d["severity"] == "error"],
    json.dumps(uni["diags"][:2]),
)
check(
    "imported const and defaults",
    r["entry"].env.roots.get("w") == 16 and uni["eng"].resolve_segs(["box", "width"]) == 8,
)
...
try:
    ...
    check(
        ...
        lock_text(u1) == lock_text(u2) and "corelib 1.0.0 " in lock_text(u1),
    )
    ...
    try:
        ...
        check(
            ...
            any(d["code"] == "E3017" for d in verify_lock(u3)),
            json.dumps(verify_lock(u3)),
        )
    finally:
    ...
    check(
        ...
        any(d["code"] == "E3016" for d in verify_lock(open_package_universe(entry))),
    )
    lock_path.write_text("")
    check(
        "missing entry is E3015",
        any(d["code"] == "E3015" for d in verify_lock(open_package_universe(entry))),
    )
finally:
    if lock_path.exists():
        lock_path.unlink()
...
check(
    ...
    any(d["code"] == "E3011" for d in bad["diags"]),
    json.dumps(bad["diags"]),
)
...
diags = load_modules(str(ROOT / "tests/packages/undeclared/main.decl"), None, und["resolver"])[
    "diags"
]
...
check(
    ...
    any(d["code"] == "E3014" for d in con["diags"]),
    json.dumps(con["diags"]),
)
...
check(
    f"fmt(fmt(x)) == fmt(x) on {idem + idem_fail} parseable files",
    idem_fail == 0,
    f"{idem_fail} failures",
)
...
Path(doc_path).write_text('{"host": "h", "port": 8}')


def run_cli(*args: str) -> tuple:
    r = subprocess.run(
        [sys.executable, "-m", "decl.runtime", "evaluate", *args],
        capture_output=True,
        text=True,
        cwd=str(ROOT / "decl-py"),
    )
...
check(
    "bound input emitted as the named root",
    code == 0 and out == '{"host":"h","port":8}',
    f"{code} {out} {err}",
)
code, out, err = run_cli(fixture, "--input", f"base={doc_path}", "--output", "copy")
check(
    "output reads the bound document",
    code == 0 and out == '{"host":"h","port":8}',
    f"{code} {out} {err}",
)
code, out, err = run_cli(fixture, "--output", "base")
check(
    "fallback-demanded input is a root",
    code == 0 and out == '{"host":"example","port":80}',
    f"{code} {out} {err}",
)
code, out, err = run_cli(fixture, "--output", "nope")
check(
    "--output naming no root exits 1",
    code == 1 and err == "no root named nope",
    f"{code} {out} {err}",
)
code, out, err = run_cli(fixture)
check(
    "a module exporting no output says so",
    code == 0 and out == "{}" and err.endswith("exports no output; --output <name> selects a root"),
    f"{code} {out} {err}",
)
code, out, err = run_cli(fixture, "--input", "nope=x.json")
check(
    "--input naming no input is a usage error",
    code == 2 and err == "no input named nope",
    f"{code} {out} {err}",
)

print("== language server over stdio ==")
server = subprocess.Popen(
    [sys.executable, "-m", "decl.runtime.lsp"],
...
check(
    ...
    init["capabilities"]["hoverProvider"] is True
    and init["capabilities"]["definitionProvider"] is True,
)
notify_server("initialized", {})
notify_server(
    "textDocument/didOpen",
    {"textDocument": {"uri": main_uri, "languageId": "decl", "version": 1, "text": "const x = \n"}},
)
d = next_diagnostics(main_uri)
check(
    "syntax error published",
    len(d["diagnostics"]) > 0 and d["diagnostics"][0]["message"] == "syntax error",
    json.dumps(d),
)
notify_server(
...
check(
    ...
    any(x.get("code") == "E4011" for x in d["diagnostics"]),
    json.dumps(d),
)
check(
    "diagnostic anchored to the name",
    d["diagnostics"][0]["range"]["start"]["line"] == 0
    and d["diagnostics"][0]["range"]["start"]["character"] > 0,
    json.dumps(d["diagnostics"][0]["range"]),
)
...
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 3}, "contentChanges": [{"text": main_src}]},
)
d = next_diagnostics(main_uri)
check("clean module publishes no diagnostics", not d["diagnostics"], json.dumps(d))
h = request(
    "textDocument/hover",
    {"textDocument": {"uri": main_uri}, "position": {"line": 1, "character": 7}},
)
check(
    "hover shows the declaration",
    bool(h) and "const top = LIMIT" in h["contents"]["value"],
    json.dumps(h),
)
h2 = request(
    "textDocument/hover",
    {"textDocument": {"uri": main_uri}, "position": {"line": 1, "character": 13}},
)
check(
    "hover follows a renamed import",
    bool(h2) and "MAX = 16" in h2["contents"]["value"],
    json.dumps(h2),
)
col = main_src.split("\n")[2].index("Service") + 2
dfn = request(
    "textDocument/definition",
    {"textDocument": {"uri": main_uri}, "position": {"line": 2, "character": col}},
)
check(
    "definition jumps across the import",
    bool(dfn) and dfn["uri"].endswith("lib.decl") and dfn["range"]["start"]["line"] == 0,
    json.dumps(dfn),
)
td = request(
    "textDocument/typeDefinition",
    {"textDocument": {"uri": main_uri}, "position": {"line": 6, "character": 14}},
)
check(
    "type definition of a value of type Service",
    bool(td) and td["uri"].endswith("lib.decl") and td["range"]["start"]["line"] == 0,
    json.dumps(td),
)
refs = request(
...
check(
    ...
    len(refs) == 6 and sum(1 for r in refs if r["uri"].endswith("lib.decl")) == 2,
    json.dumps(refs),
)
hl = request(
...
check(
    ...
    len(hl) == 2 and hl[0]["range"]["start"]["line"] == 2 and hl[1]["range"]["start"]["line"] == 6,
    json.dumps(hl),
)
c1 = request(
    "textDocument/completion",
    {"textDocument": {"uri": main_uri}, "position": {"line": 1, "character": 15}},
)
check(
    "completion of a name prefix", any(i["label"] == "LIMIT" for i in c1["items"]), json.dumps(c1)
)
notify_server(
    "textDocument/didChange",
    {
        "textDocument": {"uri": main_uri, "version": 20},
        "contentChanges": [{"text": main_src + "const e = s.\n"}],
    },
)
next_diagnostics(main_uri)
c2 = request(
    "textDocument/completion",
    {"textDocument": {"uri": main_uri}, "position": {"line": 10, "character": 12}},
)
check(
    "member completion while the text does not parse",
    ",".join(i["label"] for i in c2["items"]) == "name,port",
    json.dumps(c2),
)
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 21}, "contentChanges": [{"text": main_src}]},
)
next_diagnostics(main_uri)
syms = request("textDocument/documentSymbol", {"textDocument": {"uri": main_uri}})
check(
    "document symbols",
    ",".join(x["name"] for x in syms) == "top,s,t,first,c,d,Local",
    json.dumps(syms),
)
folds = request("textDocument/foldingRange", {"textDocument": {"uri": main_uri}})
check(
    "folding of the multi-line output",
    len(folds) == 1 and folds[0]["startLine"] == 3 and folds[0]["endLine"] == 5,
    json.dumps(folds),
)
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 22}, "contentChanges": [{"text": "const x=1\n"}]},
)
next_diagnostics(main_uri)
fmt = request(
...
check(
    ...
    len(fmt) == 1 and fmt[0]["newText"] == "const x = 1\n",
    json.dumps(fmt),
)
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 23}, "contentChanges": [{"text": main_src}]},
)
next_diagnostics(main_uri)
pr = request(
...
check(
    ...
    bool(pr) and pr["placeholder"] == "top" and pr["range"]["start"]["character"] == 6,
    json.dumps(pr),
)
rn = request(
    "textDocument/rename",
    {"textDocument": {"uri": main_uri}, "position": {"line": 2, "character": 19}, "newName": "Svc"},
)
check(
    "rename edits every module",
    bool(rn) and len(rn["changes"]) == 2 and len(rn["changes"][main_uri]) == 4,
    json.dumps(rn),
)
lenses = request("textDocument/codeLens", {"textDocument": {"uri": main_uri}})
check(
    "lenses on the outputs",
    len(lenses) == 2 and lenses[0]["command"]["command"] == "decl.evaluate",
    json.dumps(lenses),
)
ev = request("workspace/executeCommand", {"command": "decl.evaluate", "arguments": [main_uri, "s"]})
check(
    "decl.evaluate returns the document",
    bool(ev) and ev["document"] == '{"name":"a","port":8080}' and not ev["diagnostics"],
    json.dumps(ev),
)
va = request("workspace/executeCommand", {"command": "decl.validate", "arguments": [main_uri, "s"]})
check(
    "decl.validate returns the verdict",
    bool(va) and len(va["verdicts"]) == 1 and va["verdicts"][0]["errors"] == 0,
    json.dumps(va),
)
sh = request(
    "textDocument/signatureHelp",
    {"textDocument": {"uri": main_uri}, "position": {"line": 7, "character": 15}},
)
check(
    "signature help of a function call",
    bool(sh) and sh["signatures"][0]["label"] == "cap(n: int): int" and sh["activeParameter"] == 0,
    json.dumps(sh),
)
ws = request("workspace/symbol", {"query": "ca"})
check(
    "workspace symbols across the universe",
    any(x["name"] == "cap" and x["location"]["uri"].endswith("lib.decl") for x in ws),
    json.dumps(ws),
)
sr = request(
    "textDocument/selectionRange",
    {"textDocument": {"uri": main_uri}, "positions": [{"line": 6, "character": 16}]},
)
check(
    "selection ranges grow outward",
    sr[0]["range"]["start"]["character"] == 14
    and sr[0]["parent"]["range"]["start"]["character"] == 0,
    json.dumps(sr),
)
st = request("textDocument/semanticTokens/full", {"textDocument": {"uri": main_uri}})
check(
    "semantic tokens are encoded in fives",
    len(st["data"]) > 0 and len(st["data"]) % 5 == 0,
    json.dumps(st),
)
ih = request(
...
check(
    ...
    any(h["label"] == "n:" for h in ih)
    and any(h["label"] == "= 0.25 s" for h in ih)
    and any(h["label"] == ": string" for h in ih),
    json.dumps(ih),
)
ch = request(
    "textDocument/prepareCallHierarchy",
    {"textDocument": {"uri": main_uri}, "position": {"line": 7, "character": 11}},
)
inc = request("callHierarchy/incomingCalls", {"item": ch[0]})
check(
    "call hierarchy: cap is called from c",
    ch[0]["name"] == "cap" and len(inc) == 1 and inc[0]["from"]["name"] == "c",
    json.dumps(inc),
)
th = request(
    "textDocument/prepareTypeHierarchy",
    {"textDocument": {"uri": main_uri}, "position": {"line": 2, "character": 19}},
)
sub = request("typeHierarchy/subtypes", {"item": th[0]})
check(
    "type hierarchy: Service has two subtypes",
    ",".join(sorted(x["name"] for x in sub)) == "Local,Public",
    json.dumps(sub),
)
notify_server(
    "textDocument/didChange",
    {
        "textDocument": {"uri": main_uri, "version": 40},
...
check(
    ...
    any(x["title"] == 'import cap from "./lib.decl"' for x in ca),
    json.dumps(ca),
)
...
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 50}, "contentChanges": [{"text": actions_src}]},
)
next_diagnostics(main_uri)
le = request(
    "textDocument/linkedEditingRange",
    {"textDocument": {"uri": main_uri}, "position": {"line": 5, "character": 12}},
)
check(
    "linked editing of a comprehension variable",
    bool(le) and len(le["ranges"]) == 3,
    json.dumps(le),
)
lr = request(
    "textDocument/rename",
    {"textDocument": {"uri": main_uri}, "position": {"line": 5, "character": 12}, "newName": "y"},
)
...
check(
    ...
    all(
        any(x["title"] == t for x in conv)
        for t in ["annotate: int", "make hidden (x$)", "export Pair"]
    ),
    json.dumps([x["title"] for x in conv]),
)
flip = request(
    "textDocument/codeAction",
    {
...
check(
    ...
    any(
        x["title"] == "flip the comparison"
        and x["edit"]["changes"][main_uri][0]["newText"] == "4 > 3"
        for x in flip
    ),
    json.dumps(flip),
)
...
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 60}, "contentChanges": [{"text": actions2}]},
)
...
check(
    "quick fix: declare the context variable",
    any(x["title"] == "declare $parent: ref<{ ... }> on Item" for x in fix),
    json.dumps([x["title"] for x in fix]),
)
inl = request(
    "textDocument/codeAction",
    {
...
check(
    ...
    any(x["title"] == "inline K" and len(x["edit"]["changes"][main_uri]) == 3 for x in inl),
    json.dumps([x["title"] for x in inl]),
)
unit = request(
    "textDocument/codeAction",
    {
...
check(
    "assist: convert the unit literal",
    any(x["title"] == "convert to 0.25s" for x in unit),
    json.dumps([x["title"] for x in unit]),
)
reorder = request(
    "textDocument/codeAction",
    {
...
check(
    ...
    any(
        ...
        and x["edit"]["changes"][main_uri][0]["newText"] == "{\n    a: int\n    d = 1\n}"
        for x in reorder
    ),
    json.dumps([x for x in reorder if x["title"].startswith("reorder")]),
)
notify_server(
...
check(
    "inlay hints: the context variable's declared bound",
    any(h["label"] == ": ref<Owner2>" for h in ch2),
    json.dumps(ch2),
)
notify_server(
    "workspace/didChangeConfiguration",
...
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 70}, "contentChanges": [{"text": conv_src}]},
)
next_diagnostics(main_uri)
conv = request(
    "textDocument/codeAction",
    {
        "textDocument": {"uri": main_uri},
        "range": {"start": {"line": 4, "character": 13}, "end": {"line": 4, "character": 13}},
        "context": {"diagnostics": []},
    },
)
check(
    "assist: convert the if chain to match",
    any(
        x["title"] == "convert to match"
        and x["edit"]["changes"][main_uri][0]["newText"].startswith(
            "match shape {\n    (s: Circle) => s.r * 2"
        )
        for x in conv
    ),
...
inl2 = request(
    ...
    {
        "textDocument": {"uri": main_uri},
        "range": {"start": {"line": 8, "character": 4}, "end": {"line": 8, "character": 4}},
        "context": {"diagnostics": []},
    },
)
check(
    "assist: inline the derived member",
    any(
        x["title"] == "inline area"
        and len(x["edit"]["changes"][main_uri]) == 2
        and x["edit"]["changes"][main_uri][0]["newText"] == "(w * h)"
        for x in inl2
    ),
    json.dumps([x["title"] for x in inl2]),
)
dg = request(
    "textDocument/codeAction",
    {
...
check(
    "assist: declare a diagnostic for the assert",
    any(
        x["title"] == "declare a diagnostic for fits"
        and x["edit"]["changes"][main_uri][1]["newText"] == "else fits(w)"
        for x in dg
    ),
    json.dumps([x["title"] for x in dg]),
)
notify_server(
    "textDocument/didChange",
    {
        "textDocument": {"uri": main_uri, "version": 71},
...
notify_server(
    "textDocument/didChange",
    {"textDocument": {"uri": main_uri, "version": 41}, "contentChanges": [{"text": main_src}]},
)
next_diagnostics(main_uri)
tree = request(
    "workspace/executeCommand", {"command": "decl.showSyntaxTree", "arguments": [main_uri]}
)
check(
    "decl.showSyntaxTree returns the tree",
    bool(tree) and tree["tree"].startswith("(module"),
    json.dumps(tree)[:120],
)
request("shutdown", {})
notify_server("exit", {})
...
check(
    "default: the exported outputs by name", list(allv) == ["base", "prod", "dev"], str(list(allv))
)
...
check(
    ...
    byfile == {"base": {"host": "h", "port": 8}, "copy": {"host": "h", "port": 8}},
    str(byfile),
)
byvalue = api.evaluate(fixture, inputs=[("base", {"host": "v"})], outputs=["copy"])
check(
    "an input bound from a value completes",
    byvalue == {"copy": {"host": "v", "port": 80}},
    str(byvalue),
)
try:
    api.evaluate(fixture, outputs=["nope"])
    check("an unknown root is a DeclError", False)
except api.DeclError as e:
    check("an unknown root is a DeclError", str(e) == "no root named nope", str(e))
try:
    api.evaluate(fixture, inputs={"base": os.path.join(cli_tmp, "missing.json")})
    check("an unreadable document is E6004", False)
except api.DeclError as e:
    check(
        "an unreadable document is E6004",
        e.diagnostics and e.diagnostics[0]["code"] == "E6004",
        str(e.diagnostics),
    )
bad_doc = {"host": "x", "port": 70000, "workers": 100, "tls": {"enabled": True}}
try:
    api.evaluate(cfg, inputs={"deployed": bad_doc}, outputs=["deployed"])
    check("error diagnostics come back on the DeclError", False)
except api.DeclError as e:
    check(
        "error diagnostics come back on the DeclError",
        any(d["severity"] == "error" for d in e.diagnostics)
        and str(e) == e.diagnostics[0]["message"],
        str(e),
    )
v = api.validate(cfg, inputs={"deployed": bad_doc})
check(
    "validate: diagnostics of a bound document",
    any(d["severity"] == "error" for d in v),
    str(v[:2]),
)
check(
    "check: clean file -> []",
    api.check(str(ROOT / "tests/validation/types/valid/predicates.decl")) == [],
)
bad = api.check(str(ROOT / "tests/validation/types/invalid/empty_range.decl"))
check(
    "check: static error with file first",
    bad and bad[0]["code"] == "E4011" and next(iter(bad[0])) == "file",
    str(bad),
)
...
for case in sorted(p for p in (ROOT / "tests/repl").iterdir() if (p / "session.txt").exists()):
    ...
    r = subprocess.run(
        [sys.executable, "-m", "decl.cli", "repl", *entry, "--script", f"{rel}/session.txt"],
        capture_output=True,
        text=True,
        cwd=str(ROOT),
    )
    ...
    check(
        f"repl {case.name}: transcript",
        r.stdout == want,
        next(
            (
                f"line {i + 1}: expected {a!r}, got {b!r}"
    ...
    check(
        f"repl {case.name}: exit {want_code}",
...
for case in sorted(p for p in (ROOT / "tests/repl").iterdir() if (p / "session.txt").exists()):
    ...
    full = subprocess.run(
        ...
        text=True,
        cwd=str(ROOT),
        env={**os.environ, "DECL_FULL_RECOMPUTE": "1"},
    )
    check(
        f"repl {case.name}: incremental == full",
        inc.stdout == full.stdout and inc.returncode == full.returncode,
        next(
            (
                f"line {i + 1}: full {a!r}, incremental {b!r}"
                for i, (a, b) in enumerate(
                    zip(full.stdout.split("\n"), inc.stdout.split("\n"), strict=False)
                )
                if a != b
            ),
            inc.stderr[:300],
...
piped = subprocess.run(
    [
        ...
        "decl.cli",
        "repl",
        "tests/repl/documents/main.decl",
        "--input",
        "deployed=tests/repl/documents/doc.json",
        "--script",
        "-",
        "--compact",
    ],
...
check(
    "repl: --input, --script -, --compact",
    piped.stdout
    == '> deployed\n{"port":9000,"name":"doc","replicas":1,"label":"doc:9000"}\n(partial)\n',
    piped.stdout,
)
check(
    "repl: an unknown option is a usage error",
    subprocess.run(
        [sys.executable, "-m", "decl.cli", "repl", "--nope"], capture_output=True, cwd=str(ROOT)
    ).returncode
    == 2,
)
check(
    "repl: a missing script is a usage error",
    subprocess.run(
        [sys.executable, "-m", "decl.cli", "repl", "--script", os.path.join(cli_tmp, "nope.txt")],
        capture_output=True,
        cwd=str(ROOT),
    ).returncode
    == 2,
...
print("== subsume corpus: tests/subsume/cases.txt over prelude.decl ==")
from decl.runtime.subsume import structurally_empty, subsumes

_sub_parsed = parse_source((ROOT / "tests/subsume/prelude.decl").read_text(encoding="utf-8"))
assert not _sub_parsed["errors"], "prelude parse errors"
_sub_env = Env()
_sub_env.load(_sub_parsed["decls"])


def _sub_type(text: str) -> dict:
    ...
    return _sub_env.resolve(r["decls"][0]["type"])
...
for line in (ROOT / "tests/subsume/cases.txt").read_text(encoding="utf-8").split("\n"):
    ...
    if not t or t.startswith("#"):
        continue
    m = re.match(r"^(yes|no|empty|full)\s+(.+?)\s+::\s+(.+)$", t)
    if not m:
        check(f"unreadable case: {t}", False)
        continue
    verdict, name, judgment = m.groups()
    _sub_cases += 1
    if verdict in ("empty", "full"):
        check(
            name,
            structurally_empty(_sub_env, _sub_type(judgment)) == (verdict == "empty"),
            f"emptiness != {verdict == 'empty'}",
        )
        continue
    sides = judgment.split(" ⊑ ")
    if len(sides) != 2:
        check(f"unreadable judgment: {judgment}", False)
        continue
    a, b = (_sub_type(x) for x in sides)
    check(
        name,
        subsumes(_sub_env, a, b) == (verdict == "yes"),
        "expected ⊑" if verdict == "yes" else "expected not ⊑",
    )
print(f"subsume corpus: {_sub_cases} cases")
