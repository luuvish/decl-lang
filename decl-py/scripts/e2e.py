...
check("references of Service: declaration, import item, two annotations", len(refs) == 4 and sum(1 for r in refs if r["uri"].endswith("lib.decl")) == 1, json.dumps(refs))
...
c2 = request("textDocument/completion", {"textDocument": {"uri": main_uri}, "position": {"line": 7, "character": 12}})
...
check("document symbols", ",".join(x["name"] for x in syms) == "top,s,t,first", json.dumps(syms))
...
check("rename edits every module", bool(rn) and len(rn["changes"]) == 2 and len(rn["changes"][main_uri]) == 3, json.dumps(rn))
