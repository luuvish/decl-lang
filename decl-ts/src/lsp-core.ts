...
export type Io = { send: (msg: any) => void; init: (initializationOptions: any) => Promise<void>; exit: () => void };
...
export function drained(): Promise<void> { return queue; }
function send(msg: any) { io.send(msg); }
...
function readText(path: string): string { return host.readFile(path) ?? ''; }
...
function prepareRename(uri: string, pos: Pos): any {
  ...
  if (!s?.site) {
    ...
    return here ? { range: rangeOf(here), placeholder: (docs.get(uri) ?? '').split('\n')[here.sl].slice(here.sc, here.ec) } : null;
  }
...
function rename(uri: string, pos: Pos, newName: string): any {
  ...
  if (!refs.length) {
    const locs = localRanges(uri, pos);
    return locs && locs.length ? { changes: { [uri]: locs.map(l => ({ range: rangeOf(l), newText: newName })) } } : null;
  }
...
// ---------------- inlay hints ----------------
const hints = { types: true, parameterNames: true, values: false, units: true, contextVariables: false };
    ...
    if (hints.values && isDecl(x) && x.d === 'output' && x.loc && a.run.eng && a.run.entry) {
      ...
      const walk = (v: any, e: any) => {
        ...
        if (isRec(v) && e.e === 'obj') {
          ...
          if (parts.length && inRange(p)) out.push({ position: p, label: `// ${parts.join(', ')}`, paddingLeft: true });
          for (const en of e.entries) { const s = v.slots.get(en.key); if (s?.state === 'ok') walk(s.value, en.val); }
        } else if (isArr(v) && e.e === 'arr') v.items.forEach((it: any, i: number) => walk(it, e.items[i]?.expr));
        else if (isMap(v) && e.e === 'obj') for (const en of e.entries) walk(v.entries.get(en.key), en.val);
      };
      walk(a.run.entry.env.roots.get(x.name), x.expr);
    }
    if (hints.contextVariables && isExpr(x) && x.e === 'ctx' && x.loc && (x.name === '$parent' || x.name === '$root' || x.name === '$key')) {
      // the bound the enclosing type declares for the variable
      const decl: any = m.decls.find(d => d.d === 'type' && d.loc && d.loc.sl <= x.loc.sl && x.loc.el <= d.loc.el);
      ...
      if (ctxm?.type?.loc && inRange(p)) out.push({ position: p, label: `: ${srcOf(text, ctxm.type.loc)}`, kind: 1 });
    }
...
const recordBodyOf = (decl: any): any => decl?.d === 'type' ? (decl.type.k === 'record' ? decl.type : decl.type.k === 'named' && decl.type.ext?.k === 'record' ? decl.type.ext : null) : null;
...
function codeActions(uri: string, range: Range, diagnostics: any[]): any[] {
  ...
  if (a && !errors.length) {
    ...
    const touches = (d: any) => d.range && !(d.range.end.line < range.start.line || d.range.start.line > range.end.line);
    for (const d of (diagnostics ?? []).filter(touches)) {
      ...
      if (mm) {
        ...
        for (const [ns, entry] of m.env.namespaces) {
          ...
          const n = hit && [hit.node, ...[...hit.parents].reverse()].find(x => isExpr(x) && x.e === 'name' && x.name === name);
          if (n?.loc) out.push({ title: `qualify as ${ns}.${name}`, kind: 'quickfix', diagnostics: [d], edit: { changes: { [uri]: [{ range: rangeOf(n.loc), newText: `${ns}.${name}` }] } } });
        }
      }
      mm = /^member ([A-Za-z_][A-Za-z0-9_]*) is not declared on ([A-Za-z_][A-Za-z0-9_]*)$/.exec(d.message ?? '');
      if (mm) {
        ...
        const decl: any = site?.decl;
        const body = decl?.d === 'type' ? (decl.type.k === 'record' ? decl.type : decl.type.k === 'named' && decl.type.ext?.k === 'record' ? decl.type.ext : null) : null;
        if (site && body) {
          const hit = nodeAt(decls, d.range.start);
          const chain = hit ? [hit.node, ...[...hit.parents].reverse()] : [];
          let entry: any = chain.find(n => isExpr(n) && n.e === 'obj')?.entries.find((en: any) => en.key === name);
          if (!entry && hit) { const objs: any[] = []; findAll(hit.node, n => isExpr(n) && n.e === 'obj', objs); entry = objs.flatMap(o => o.entries).find((en: any) => en.key === name); }
          ...
          const widen = (rt: any): any => rt?.t === 'lit' ? { t: 'prim', name: typeof rt.v === 'string' ? 'string' : typeof rt.v === 'boolean' ? 'bool' : typeof rt.v === 'bigint' ? 'int' : typeof rt.v === 'number' ? 'float' : 'null' } : rt;
          const memberType = ty?.rt ? typeText(widen(ty.rt)) : 'any';
          const last = body.members[body.members.length - 1];
          const at = last?.loc ? { line: last.loc.el, character: last.loc.ec } : { line: body.loc.sl, character: body.loc.sc + 1 };
          out.push({ title: `declare ${name}: ${memberType} on ${typeName}`, kind: 'quickfix', diagnostics: [d], isPreferred: true,
            edit: { changes: { [uriOf(site.module.path)]: [{ range: { start: at, end: at }, newText: last?.loc ? `\n    ${name}: ${memberType}` : ` ${name}: ${memberType}` }] } } });
        }
      }
      if (/^member access on a maybe-absent expression/.test(d.message ?? '')) {
        const hit = nodeAt(decls, d.range.start);
        const n = hit && [hit.node, ...[...hit.parents].reverse()].find(x => isExpr(x) && x.e === 'member' && !x.safe);
        if (n?.loc) {
          const tok = memberTokenLoc(text, n);
          out.push({ title: 'use ?.', kind: 'quickfix', diagnostics: [d], isPreferred: true, edit: { changes: { [uri]: [{ range: { start: { line: tok.sl, character: tok.sc - 1 }, end: { line: tok.sl, character: tok.sc } }, newText: '?.' }] } } });
        }
      }
      if (/^maybe-absent expression consumed/.test(d.message ?? '')) {
        ...
        if (n && isExpr(n) && n.loc) {
          ...
          out.push({ title: 'supply a fallback with ??', kind: 'quickfix', diagnostics: [d], edit: { changes: { [uri]: [{ range: { start: end, end }, newText: ' ?? null' }] } } });
        }
      }
      if (/^`\?\?` mixed with/.test(d.message ?? '')) {
        ...
        const find = (x: any, parentOp: string | null) => {
          ...
          if (Array.isArray(x)) { x.forEach(y => find(y, parentOp)); return; }
          if (isExpr(x) && x.e === 'bin' && x.loc && parentOp && ((x.op === '??' && logical(parentOp)) || (logical(x.op) && parentOp === '??'))) { target = x; return; }
          const op = isExpr(x) && x.e === 'bin' ? x.op : null;
          for (const [k, v] of Object.entries(x)) if (k !== 'loc' && v && typeof v === 'object') find(v, op);
        };
        if (hit) find(hit.node, null);
        if (target) out.push({ title: 'parenthesize the ?? expression', kind: 'quickfix', diagnostics: [d], isPreferred: true,
          edit: { changes: { [uri]: [{ range: { start: { line: target.loc.sl, character: target.loc.sc }, end: { line: target.loc.sl, character: target.loc.sc } }, newText: '(' },
            { range: { start: { line: target.loc.el, character: target.loc.ec }, end: { line: target.loc.el, character: target.loc.ec } }, newText: ')' }] } } });
      }
      if (/^`match` is not exhaustive/.test(d.message ?? '')) {
        const hit = nodeAt(decls, d.range.start);
        const n = hit && [hit.node, ...[...hit.parents].reverse()].find(x => isExpr(x) && x.e === 'match');
        const subject = n ? t.types.get(n.subject)?.rt : null;
        const arms: string[] = subject?.t === 'union' ? subject.arms.filter((r: any) => r.t === 'rec' && r.name).map((r: any) => r.name) : [];
        const covered = new Set(n ? n.arms.map((arm: any) => arm.type?.k === 'named' ? arm.type.name : '') : []);
        ...
        if (n?.loc && missing.length) {
          ...
          out.push({ title: `add the missing arm${missing.length > 1 ? 's' : ''}: ${missing.join(', ')}`, kind: 'quickfix', diagnostics: [d], isPreferred: true,
            edit: { changes: { [uri]: [{ range: { start: at, end: at }, newText: missing.map(x => `${indent}(v: ${x}) => null\n`).join('') + indent.slice(4) }] } } });
        }
      }
      mm = /^(\$[a-z]+) used without a context declaration in ([A-Za-z_][A-Za-z0-9_]*)$/.exec(d.message ?? '');
      if (mm) {
        ...
        if (site && body?.loc) {
          ...
          const at = first?.loc ? { line: first.loc.sl, character: first.loc.sc } : { line: body.loc.sl, character: body.loc.sc + 1 };
          const indent = first?.loc ? '' : ' ';
          out.push({ title: `declare ${variable}: ${bound} on ${typeName}`, kind: 'quickfix', diagnostics: [d], isPreferred: true,
            edit: { changes: { [uriOf(site.module.path)]: [{ range: { start: at, end: at }, newText: first?.loc && first.loc.sl > body.loc.sl ? `${variable}: ${bound}\n${' '.repeat(first.loc.sc)}` : `${indent}${variable}: ${bound}, ` }] } } });
        }
      }
      mm = /^(\$[a-z]+) declaration must be ref<\.\.\.> \(([A-Za-z_][A-Za-z0-9_]*)\)$/.exec(d.message ?? '');
      if (mm) {
        ...
        if (site && ctxm?.type?.loc) {
          ...
          out.push({ title: `declare ${variable} as ref<${srcOf(textOf(a, site.module), tl)}>`, kind: 'quickfix', diagnostics: [d], isPreferred: true,
            edit: { changes: { [uriOf(site.module.path)]: [{ range: rangeOf(tl), newText: `ref<${srcOf(textOf(a, site.module), tl)}>` }] } } });
        }
      }
      mm = /^(?:illegal member-kind transition for|override widens inherited member) ([A-Za-z_][A-Za-z0-9_]*)[^(]*\(([A-Za-z_][A-Za-z0-9_]*)\)$/.exec(d.message ?? '');
      if (mm) {
        ...
        const parent = decl?.type?.k === 'named' ? memberSite(a, site!.module, m.env.resolve ? safeResolve(site!.module, decl.type.name) : null, member) : null;
        if (site && own && parent?.member?.loc) {
          const parentText = srcOf(textOf(a, parent.module), parent.member.loc).replace(/,\s*$/, '');
          out.push({ title: `use the parent's declaration: ${parentText}`, kind: 'quickfix', diagnostics: [d], isPreferred: true,
            edit: { changes: { [uriOf(site.module.path)]: [{ range: rangeOf(own.loc), newText: parentText }] } } });
        }
      }
      mm = /^record union arms not discriminable in ([A-Za-z_][A-Za-z0-9_]*)$/.exec(d.message ?? '');
      if (mm) {
        ...
        for (const arm of arms) {
          ...
          (changes[uriOf(as.module.path)] ??= []).push({ range: { start: at, end: at }, newText: multi ? `kind: "${arm.name}"\n${' '.repeat(first.loc.sc)}` : `${first?.loc ? '' : ' '}kind: "${arm.name}", ` });
          n++;
        }
        if (n) out.push({ title: `add a discriminant \`kind\` to the arms of ${mm[1]}`, kind: 'quickfix', diagnostics: [d], isPreferred: true, edit: { changes } });
      }
      mm = /^derived member ([A-Za-z_][A-Za-z0-9_]*) restated with a differing value/.exec(d.message ?? '');
      if (mm) {
        // a document supplies it: make it defaulted (`x?: T = e`) where it is declared with a type
        const member = mm[1];
        for (const decl of m.decls as any[]) {
          const body = recordBodyOf(decl);
          const own = body?.members.find((x: any) => x.m === 'derived' && x.name === member && x.type && x.loc);
          if (!own) continue;
          const r = memberRange(text, own, member);
          out.push({ title: `make ${decl.name}.${member} defaulted (x?: T = e)`, kind: 'quickfix', diagnostics: [d], edit: { changes: { [uri]: [{ range: { start: { line: r.el, character: r.ec }, end: { line: r.el, character: r.ec } }, newText: '?' }] } } });
        }
      }
    ...
    const chain = hit ? [hit.node, ...[...hit.parents].reverse()] : [];
    const one = (title: string, kind: string, edits: any[], target = uri) => out.push({ title, kind, edit: { changes: { [target]: edits } } });
    ...
    const unannotated = chain.find(n => (isMember(n) && n.m === 'derived' && !n.type) || (isDecl(n) && n.d === 'const' && !n.type));
    if (unannotated) {
      const ty = t.types.get(unannotated.expr);
      if (ty?.rt) {
        const r = isMember(unannotated) ? memberRange(text, unannotated, unannotated.name) : nameRange(text, unannotated, unannotated.name);
        one(`annotate: ${typeText(ty.rt)}`, 'refactor.rewrite', [insertAt({ line: r.el, character: r.ec + (unannotated.hidden ? 1 : 0) }, `: ${typeText(ty.rt)}`)]);
      }
    }
    // convert a member's kind: derived <-> hidden, defaulted <-> derived, optional <-> required
    const member = chain.find(n => isMember(n) && (n.m === 'derived' || n.m === 'value') && n.loc);
    if (member) {
      const r = memberRange(text, member, member.name);
      const afterName = { line: r.el, character: r.ec };
      if (member.m === 'derived') {
        if (member.hidden) one('make visible (derived)', 'refactor.rewrite', [{ range: { start: afterName, end: { line: r.el, character: r.ec + 1 } }, newText: '' }]);
        else one('make hidden (x$)', 'refactor.rewrite', [insertAt(afterName, '$')]);
        if (member.type) one('make defaulted (x?: T = e)', 'refactor.rewrite', [insertAt(afterName, '?')]);
      } else if (member.dflt) {
        one('make derived (x: T = e)', 'refactor.rewrite', [{ range: { start: afterName, end: { line: r.el, character: r.ec + 1 } }, newText: '' }]);
      } else if (member.opt) {
        one('make required', 'refactor.rewrite', [{ range: { start: afterName, end: { line: r.el, character: r.ec + 1 } }, newText: '' }]);
      } else {
    ...
    if (decl?.loc && typeof decl.name === 'string' && !decl.exported && decl.d !== 'import' && decl.d !== 're_export')
      one(`export ${decl.name}`, 'refactor.rewrite', [insertAt({ line: decl.loc.sl, character: decl.loc.sc }, 'export ')]);
    if (decl?.d === 'type') {
      let rt: any = null;
      try { rt = m.env.resolve(decl.type); } catch { rt = null; }
      if (rt?.t === 'rec') {
        const req = rt.members.filter((x: any) => x.kind === 'req').map((x: any) => `${x.name}: ${placeholderFor(x.type)}`);
        ...
        one(`generate an output of ${decl.name}`, 'refactor.rewrite', [insertAt(end, `${lead}output ${lower}: ${decl.name} = { ${req.join(', ')}${req.length ? ' ' : ''}}\n`)]);
        one(`generate an input of ${decl.name}`, 'refactor.rewrite', [insertAt(end, `${lead}input ${lower}: ${decl.name}\n`)]);
      }
    }
    if (diagnostics.length && !/^\/\/ @expect-/.test(lines[0] ?? '')) {
      ...
      const phase = code.startsWith('E1') || code.startsWith('E2') ? 'parsing' : code.startsWith('E5') || code.startsWith('E6') || /^[A-Z][A-Za-z0-9_]*\./.test(code) ? 'binding' : 'checking';
      one('generate the fixture header (@expect-phase / @expect-error)', 'refactor.rewrite', [insertAt({ line: 0, character: 0 }, `// @expect-phase: ${phase}\n// @expect-error: ${code}\n`)]);
    }
    ...
    if (obj?.loc) {
      const owner: any = chain.find(n => isDecl(n) && (n.d === 'output' || n.d === 'input') && n.type);
      let rt: any = null;
      try { rt = owner && owner.expr === obj ? m.env.resolve(owner.type) : t.types.get(obj)?.rt ?? null; } catch { rt = null; }
      if (rt?.t === 'rec') {
        ...
        if (missing.length) {
          ...
          const edit = last?.val?.loc ? insertAt({ line: last.val.loc.el, character: last.val.loc.ec }, `, ${fill}`) : insertAt({ line: obj.loc.sl, character: obj.loc.sc + 1 }, ` ${fill}`);
          one(`fill the required members: ${missing.map((x: any) => x.name).join(', ')}`, 'refactor.rewrite', [edit]);
        }
      }
    }
    ...
    if (decl?.d === 'const' && decl.loc && decl.expr?.loc) {
      ...
      const refs = references(uri, { line: nr.sl, character: nr.sc }, false).filter(r => r.module.path === m.path);
      if (refs.length) {
        const src = srcOf(text, decl.expr.loc);
        const edits = refs.map(r => ({ range: rangeOf(r.loc), newText: isExpr(decl.expr) && ['name', 'lit', 'unitlit', 'call', 'member', 'paren'].includes(decl.expr.e) ? src : `(${src})` }));
        edits.push({ range: { start: { line: decl.loc.sl, character: 0 }, end: { line: decl.loc.el + 1, character: 0 } }, newText: '' });
    ...
    const inlineRecord = chain.find(n => isType(n) && n.k === 'record' && n.loc && n !== ownBody)
      ?? (() => { const mem = chain.find(n => isMember(n) && n.m === 'value' && n.type?.k === 'record' && n.type.loc); return mem?.type; })();
    if (inlineRecord && decl?.loc) {
      one('extract to a named type', 'refactor.extract', [
        { range: { start: { line: decl.loc.sl, character: 0 }, end: { line: decl.loc.sl, character: 0 } }, newText: `type Extracted = ${srcOf(text, inlineRecord.loc)}\n` },
    ...
    if (unitLit) {
      try {
        ...
        const base = m.env.baseUnitOf.get(u.key) ?? u.key;
        if (base !== unitLit.unit) one(`convert to ${unitLit.num * u.toBase}${base}`, 'refactor.rewrite', [{ range: rangeOf(unitLit.loc), newText: `${unitLit.num * u.toBase}${base}` }]);
      } catch { /* an unknown unit is a diagnostic */ }
    }
    ...
    if (body?.loc && body.members.length > 1 && body.members.every((x: any) => x.loc)) {
      const rank = (x: any) => x.m === 'context' ? 0 : x.m === 'value' ? (x.dflt ? 3 : x.opt ? 2 : 1) : x.m === 'derived' ? (x.hidden ? 5 : 4) : 6;
      const sorted = [...body.members].map((x: any, i: number) => ({ x, i })).sort((p, q) => rank(p.x) - rank(q.x) || p.i - q.i);
    ...
    if (cmp) {
      ...
      one('flip the comparison', 'refactor.rewrite', [{ range: rangeOf(cmp.loc), newText: `${srcOf(text, cmp.r.loc)} ${flipped[cmp.op]} ${srcOf(text, cmp.l.loc)}` }]);
    }
    // extract the selected expression: into a constant (a constant expression), or a derived member (inside a record body)
    const selected = range.start.line !== range.end.line || range.start.character !== range.end.character
      ? chain.find(n => isExpr(n) && n.loc && n.loc.sl === range.start.line && n.loc.sc === range.start.character && n.loc.el === range.end.line && n.loc.ec === range.end.character) : null;
    if (selected && !(isExpr(selected) && selected.e === 'name')) {
      const mentionsName = (x: any): boolean => !!x && typeof x === 'object' && (Array.isArray(x) ? x.some(mentionsName) : (isExpr(x) && (x.e === 'name' || x.e === 'ctx' || x.e === 'referrers')) || Object.entries(x).some(([k, v]) => k !== 'loc' && mentionsName(v)));
      ...
      if (!mentionsName(selected) && enclosingDecl?.loc)
        one('extract to a constant', 'refactor.extract', [insertAt({ line: enclosingDecl.loc.sl, character: 0 }, `const extracted = ${src}\n`), { range: rangeOf(selected.loc), newText: 'extracted' }]);
      if (enclosingMember && enclosingMember.m !== 'context') {
        const indent = lines[enclosingMember.loc.sl].match(/^ */)?.[0] ?? '';
        one('extract to a derived member', 'refactor.extract', [insertAt({ line: enclosingMember.loc.sl, character: 0 }, `${indent}extracted = ${src}\n`), { range: rangeOf(selected.loc), newText: 'extracted' }]);
      }
    }
  }
  return out;
...
function require_readdir(dir: string): string[] { return host.readDir(dir); }
function safeResolve(m: Module, typeName: string): any { try { return m.env.resolve({ k: 'named', name: typeName, args: [] } as any); } catch { return null; } }
function findAll(x: any, pred: (n: any) => boolean, out: any[]) {
  if (!x || typeof x !== 'object') return;
  if (Array.isArray(x)) { x.forEach(y => findAll(y, pred, out)); return; }
  if (pred(x)) out.push(x);
  for (const [k, v] of Object.entries(x)) if (k !== 'loc' && v && typeof v === 'object') findAll(v, pred, out);
}
...
const bindsName = (n: any, name: string): boolean =>
  (isExpr(n) && ((n.e === 'comp' || n.e === 'mapcomp') && n.clauses.some((c: any) => c.v === name)
...
function bindingLocs(text: string, scope: any, name: string): Loc[] {
  ...
  const lines = text.split('\n');
  const out: Loc[] = [];
  const at = (offset: number) => {   // an offset in `src` -> a Loc
    let line = scope.loc.sl, col = scope.loc.sc;
    for (let i = 0; i < offset; i++) { if (src[i] === '\n') { line++; col = 0; } else col++; }
    return { sl: line, sc: col, el: line, ec: col + name.length };
  };
  const esc = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const patterns = isDecl(scope) ? [new RegExp(`\\(([^)]*)\\b(${esc})\\b`)]
    : scope.e === 'lambda' ? [new RegExp(`\\b(${esc})\\b(?=[^=]*=>)`)]
    : scope.e === 'match' ? [new RegExp(`\\(\\s*(${esc})\\b`, 'g')]
    : [new RegExp(`\\bfor\\s+(${esc})\\b`, 'g')];
  // every pattern ends with the name itself: the token is the tail of the match
  for (const re of patterns) {
    if (re.global) { let mm; while ((mm = re.exec(src))) out.push(at(mm.index + mm[0].length - name.length)); }
    else { const mm = re.exec(src); if (mm) out.push(at(mm.index + mm[0].length - name.length)); }
  }
  void lines;
  return out;
}
function localRanges(uri: string, pos: Pos): Loc[] | null {
  ...
  if (!hit) return null;
  let name: string | null = null, scope: any = null;
  if (isExpr(hit.node) && hit.node.e === 'name') {
    ...
    let mm;
    while ((mm = w.exec(line))) if (mm.index <= pos.character && pos.character <= mm.index + mm[0].length) { name = mm[0]; break; }
    if (!name) return null;
  ...
  const uses = (x: any) => {
    if (!x || typeof x !== 'object') return;
    if (Array.isArray(x)) { x.forEach(uses); return; }
  ...
  for (const [k, v] of Object.entries(scope)) if (k !== 'loc' && v && typeof v === 'object') uses(v);
  const key = (l: Loc) => `${l.sl}:${l.sc}`;
  const seen = new Set<string>();
  return locs.filter(l => !seen.has(key(l)) && seen.add(key(l))).sort((p, q) => p.sl - q.sl || p.sc - q.sc);
}
...
async function handle(msg: any) {
  ...
  switch (method) {
    ...
    case 'decl/files': {
      ...
      for (const u of params?.remove ?? []) { const p = pathOf(u); if ('files' in host) (host as any).files.delete(p); }
      analyses.clear();
