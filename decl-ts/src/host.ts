...
export function memoryHost(files: Map<string, string>, opts: { cwd?: string; uriPrefix?: string } = {}): Host & { files: Map<string, string>; uriPrefix: string } {
  const h = {
    files,
    ...
    readDir: (p: string) => {
      ...
      for (const f of files.keys()) if (f.startsWith(dir)) out.add(f.slice(dir.length).split('/')[0]);
      return [...out];
    },
    isDir: (p: string) => { const dir = normalize(p).replace(/\/$/, '') + '/'; for (const f of files.keys()) if (f.startsWith(dir)) return true; return false; },
    exists: (p: string) => files.has(normalize(p)) || h.isDir(p),
    writeFile: (p: string, text: string) => { files.set(normalize(p), text); },
    cwd: () => opts.cwd ?? '/',
    pathOf: (uri: string) => {
    ...
    uriOf: (p: string) => h.uriPrefix + p.split('/').map(s => encodeURIComponent(s)).join('/'),
    env: () => undefined,
  };
  return h;
}
...
export function resolvePath(...parts: string[]): string {
  let p = '';
  for (const part of parts) { if (!part) continue; p = part.startsWith('/') || !p ? part : `${p}/${part}`; }
...
export function sha256Hex(chunks: (string | Uint8Array)[]): string {
  ...
  let off = 0;
  for (const p of parts) { padded.set(p, off); off += p.length; }
  padded[len] = 0x80;
