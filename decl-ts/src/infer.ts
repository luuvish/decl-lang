...
export const STD: Record<string, { arity: number; ret: RT | null }> = {
  ...
  'array.sort': { arity: 1, ret: null },
  'array.sort_by': { arity: 2, ret: null },
  'array.unique': { arity: 1, ret: null },
  'array.reverse': { arity: 1, ret: null },
...
export function infer(cx: ICtx, e: Expr): Ty {
  const prev = cx.pos.at;
  cx.pos.at = e;
  try {
    const ty = infer0(cx, e);
...
export function resolveIn(env: Env, name: string): Target | null {
  ...
  if (env.namespaces.has(name)) return { kind: 'namespace', env, name };
  if (env.typeAsts.has(name)) return { kind: 'type', env, name };
  if (env.diags.has(name)) return { kind: 'diagnostic', env, name };
  return null;
}
