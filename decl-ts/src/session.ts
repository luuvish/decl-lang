...
export function parseDecl(text: string): { decl: Decl; name: string } {
  ...
  if (errors.length || decls.length !== 1)
    throw new SessionError(`cannot parse declaration: ${text.trim().split('\n')[0]}`);
  const d: any = decls[0];
  const name =
...
function parseDoc(text: string, what: string): any {
  try {
    ...
    throw new SessionError(`${what} is not well-formed JSON`);
  }
}
...
export function docJson(v: any): string {
  ...
  if (v && v.__jobj)
    return `{${v.entries.map(([k, x]: any) => `${JSON.stringify(k)}:${docJson(x)}`).join(',')}}`;
...
function docStep(v: any, seg: Seg): any {
  const k = segText(seg);
  if (v && v.__jobj) {
    const e = v.entries.find(([kk]: any) => kk === k);
...
export function prettyJson(compact: string): string {
  ...
  while (i < compact.length) {
    ...
    if (c === '"') {
      ...
      out += compact.slice(i, j + 1);
      i = j + 1;
      continue;
    }
    if (c === '{' || c === '[') {
      const close = c === '{' ? '}' : ']';
      if (compact[i + 1] === close) {
        out += c + close;
        i += 2;
        continue;
      }
...
export class Session {
  ...
  get entryAbs(): string {
    return this.entryPath ?? absPath(Session.SCRATCH);
  }
  get entryName(): string {
    return this.entryPath ? basename(this.entryPath) : Session.SCRATCH;
  }
  ...
  reloadOp(): Op {
    return { op: 'reload', snapshot: this.snapshotFromDisk() };
  }
  ...
  private applyTo(st: State, op: Op) {
    switch (op.op) {
      case 'bind': {
        ...
        if (!modules.some((m) => m.env.inputs.has(op.name)))
          throw new SessionError(`no input named ${op.name}`);
        const doc =
          op.src.kind === 'expr'
      ...
      case 'edit':
        ...
        return;
      case 'declare':
        st.decls.delete(op.name);
        st.outputs.delete(op.name);
      ...
      case 'output':
        ...
        return;
      case 'drop':
        if (!st.decls.delete(op.name) && !st.outputs.delete(op.name))
          throw new SessionError(`no session declaration named ${op.name}`);
        return;
      case 'reload':
  ...
  private evalToDoc(st: State, exprText: string): any {
    ...
    return this.scratch(r, (eng) => {
      try {
        ...
        if (e instanceof Taint || e instanceof DeferSig)
          throw new SessionError('the value is invalid');
        throw e;
      }
    });
  }
  ...
  private edit(st: State, op: Extract<Op, { op: 'edit' }>) {
    ...
    try {
      ...
      throw new SessionError(`bad path ${op.path}`);
    }
    if (typeof segs[0] !== 'string' || segs[0] === '')
      throw new SessionError(`bad path ${op.path}`);
    const root = segs[0];
  ...
  private documentOf(st: State, root: string): Document {
    ...
    if (!inputMod && !outputMod)
      throw new SessionError(
        st.outputs.has(root)
          ? `${root} is a session output; edit the roots it reads`
          : `no root named ${root}`,
      );
    ...
    const d: Document = {
      origin: inputMod ? 'fallback' : 'detached',
      doc,
      base: docClone(doc),
      edited: false,
    };
  ...
  private build(st: State): { modules: Module[]; entry: Module | null; diags: Diag[] } {
    ...
    if (text !== undefined) {
      text = detachOutputs(
        text,
        [...st.documents.entries()].filter(([, d]) => d.origin === 'detached').map(([n]) => n),
      );
  ...
  run(st: State = this.state, mode: 'check' | 'lazy' | 'full' = 'full'): Run {
    ...
    if (mode === 'full')
      this.last = r.eng ? { key: this.universeKey(st), docs: this.docKeys(st), run: r } : null;
    return r;
  }
  private universeKey(st: State): string {
    const detached = [...st.documents.entries()]
      .filter(([, d]) => d.origin === 'detached')
      .map(([n]) => n)
      .sort();
  ...
  private stepFrom(st: State): Run | null {
    ...
    if (!changed.size) {
      this.lastTiming = last.run.timing;
      return last.run;
    }
    const t0 = now();
    const r = last.run;
  ...
  private runFresh(st: State, mode: 'check' | 'lazy' | 'full'): Run {
    ...
    const finish = () => {
      out.timing.total = now() - t0;
      this.lastTiming = out.timing;
      return out;
    };
    ...
    for (const [name, o] of st.outputs) {
      const taken = b.modules.some(
        (m) => m.env.inputs.has(name) || m.env.outputs.some((x) => x.name === name),
      );
      if (taken) {
        out.sessionChecks.push({
          severity: 'error',
          code: 'E3018',
          message: `root ${name} is already declared by the universe`,
          path: name,
        });
        continue;
      }
      ...
      try {
        ...
        const cx = this.sessionCtx(
          st,
          entry.env,
          (code, msg) =>
            out.sessionChecks.push({ severity: 'error', code, message: msg, path: name }),
          name,
        );
    ...
    if (
      mode === 'check' ||
      (mode === 'full' && out.checks.some((c) => c.diag.severity === 'error'))
    )
      return finish();
  ...
  private sessionCtx(
    ...
    for (const [name, o] of st.outputs) {
      ...
      try {
        ...
        const quiet = makeCtx(env, () => {});
        quiet.vars = new Map(cx.vars);
  ...
  private scratch<T>(r: Run, f: (eng: Engine, env: Env) => T): T {
    ...
    try {
      ...
      const under = (p: string) =>
        demanded.some((k) => p === k || p.startsWith(k + '.') || p.startsWith(k + '['));
      for (const k of demanded) {
        env.roots.delete(k);
        eng.failedInputs.delete(k);
      ...
      env.registry.splice(
        ...
        ...env.registry.filter(
          (inst, i) =>
            (i < reg && !under(pathStr(inst.path))) ||
            (i >= reg && inst.path[0] !== '_' && !under(pathStr(inst.path))),
        ),
      );
  ...
  evaluateExpr(text: string): {
    ...
    return this.scratch(r, (eng, env) => {
      ...
      const arising = () =>
        sortDiags([...all.slice(0, from).filter((d) => named.has(d.path)), ...all.slice(from)]);
      try {
        let v = eng.ev(expr, sc);
        v = eng.materialize(v, ['_'], null, sc);
        eng.forceAll(v, true);
        ...
        if (e instanceof EvalErr)
          return { value: null, diags: arising(), error: { code: e.code, message: e.message } };
        if (e instanceof Taint || e instanceof DeferSig)
          return { value: null, diags: arising(), error: { message: '' } };
        throw e;
      }
    });
  }
  ...
  roots(): RootInfo[] {
    ...
    const out: RootInfo[] = [];
    const rel = (p: string) =>
      p === this.entryAbs ? this.entryName : relative(dirname(this.entryAbs), p);
    for (const m of b.modules) {
      ...
      const decls =
        m.path === this.entryAbs
          ? parseSource(this.state.snapshot.get(m.path) ?? '').decls
          : m.decls;
      for (const decl of decls) {
        if (decl.d === 'output') {
          ...
          out.push({
            ...
            session: false,
            binding: d?.origin === 'detached' ? 'detached' : '',
            detail: '',
            edited: !!d?.edited,
          });
  ...
  hasRoot(name: string): boolean {
    return this.allRootNames().includes(name);
  }
  ...
  check(): { file: string; diag: Diag }[] {
    ...
    return [
      ...r.loadDiags.map((d) => ({ file: this.entryAbs, diag: d })),
      ...r.checks,
      ...r.sessionChecks.map((d) => ({ file: this.entryAbs, diag: d })),
    ];
  }
  ...
  evaluate(names: string[]): {
    run: Run;
    docs: { name: string; json: string | null }[];
    exported: boolean;
  } {
  ...
  validate(names: string[]): {
    run: Run;
    verdicts: { name: string; errors: number; warnings: number }[];
    diags: Diag[];
  } {
    for (const n of names) if (!this.hasRoot(n)) throw new SessionError(`no root named ${n}`);
    const r = this.run(this.state, 'full');
    const want = names.length ? names : r.entry ? [...r.entry.env.roots.keys()] : [];
    const diags = r.diags.filter(
      (d) => want.some((n) => isRootDiag(d, n)) || (!d.path && names.length === 0),
    );
  ...
  typeOf(text: string): { type: string; maybeAbsent: boolean; diags: Diag[] } {
    ...
    const diags: Diag[] = [];
    const cx = this.sessionCtx(this.state, b.entry.env, (code, message) =>
  ...
  pathOf(text: string): string {
    ...
    return this.scratch(r, () => {
      try {
        let segs = eng.evalPlace(expr, sc);
        // a scalar member or element is a place too: its container's place, one step down
        if (!segs && (expr.e === 'member' || expr.e === 'index')) {
          const base = eng.evalPlace((expr as any).x, sc);
          if (base) {
            const step =
              expr.e === 'member'
                ? expr.name
                : (() => {
                    const i = eng.ev((expr as any).i, sc);
                    return typeof i === 'bigint' ? Number(i) : mapKey(i);
                  })();
            segs = [...base, step];
          }
        }
        ...
        if (e instanceof EvalErr) throw new SessionError(e.message);
        if (e instanceof Taint || e instanceof DeferSig)
          throw new SessionError('the place is invalid');
        throw e;
      }
    });
  }
  ...
  docOf(name: string): string[] {
    ...
    if (!member && this.state.outputs.has(head)) {
      const o = this.state.outputs.get(head)!;
      return [`${head}${o.type ? `: ${o.type}` : ''} = ${o.expr}`];
    }
    const b = this.build(this.state);
    if (!b.entry) throw new SessionError(b.diags[0]?.message ?? 'the universe did not load');
    let mod: Module | null = b.entry,
      target = head;
    if (!b.entry.decls.some((d) => (d as any).name === head)) {
      const im = b.entry.env.imports.get(head);
      if (im) {
        mod = b.modules.find((m) => m.env === im.env) ?? null;
        target = im.name;
      } else mod = null;
    }
    ...
    if (member) {
      ...
      const picked: string[] = [];
      body.forEach((l, i) => {
        if (new RegExp(`^\\s*${member}\\$?\\??\\s*[:=]`).test(l)) {
          let j = i;
          const ds: string[] = [];
          while (j > 0 && /^\s*\/\/\//.test(body[j - 1])) {
            j--;
            ds.unshift(body[j].trim());
          }
          picked.push(...ds, l.trim());
        }
      });
      void re;
  ...
  trace(pathText: string): string[] {
    let segs: Seg[];
    try {
      segs = parsePath(pathText, '');
    } catch {
      throw new SessionError(`bad path ${pathText}`);
    }
    ...
    const walk = (segs: Seg[], depth: number) => {
      ...
      const last = segs[segs.length - 1];
      const slot =
        parent && isRec(parent) && typeof last === 'string' ? parent.slots.get(last) : undefined;
      if (slot) {
        const kind =
          slot.kind === 'der'
            ? 'derived'
        ...
        const supplied =
          slot.kind === 'req' ||
          slot.kind === 'opt' ||
          (slot.kind === 'dflt' && parent.entryOrder.includes(last));
        if (slot.state === 'invalid') {
          lines.push(`${ind}${path}  (invalid)`);
          for (const d of own) lines.push(`${ind}  ${fmtDiag(d)}`);
          if (!own.length && m?.expr)
            for (const rd of readsOf(m.expr)) {
              const s = this.readSegs(eng, parent, rd, r.entry!);
              if (s) walk(s, depth + 1);
            }
          return;
        }
        if (slot.state === 'absent') {
          lines.push(`${ind}${path}  absent`);
          return;
        }
        lines.push(
          `${ind}${path} = ${short(slot.value)}  (${supplied ? 'supplied' : kind}${m?.expr && !supplied ? `: ${exprText(m.expr)}` : ''})`,
        );
        if (!supplied && m?.expr && depth < 6)
          for (const rd of readsOf(m.expr)) {
            const s = this.readSegs(eng, parent, rd, r.entry!);
            if (s) walk(s, depth + 1);
            else lines.push(`${ind}  ${exprText(rd)}  (not a place)`);
          }
        return;
      }
      ...
      if (v === undefined) {
        if (r.diags.some((d) => d.severity === 'error' && isRootDiag(d, path))) {
          lines.push(`${ind}${path}  (invalid)`);
          for (const d of r.diags.filter((d) => isRootDiag(d, path)))
            lines.push(`${ind}  ${fmtDiag(d)}`);
        } else lines.push(`${ind}${path}  nothing there`);
        return;
      }
      lines.push(
        `${ind}${path} = ${short(v)}  (${segs.length === 1 ? (this.state.documents.has(root) ? 'document' : 'root literal') : 'supplied'})`,
      );
      for (const d of own) lines.push(`${ind}  ${fmtDiag(d)}`);
    };
    walk(segs, 0);
  ...
  private valueAt(eng: Engine, entry: Module, segs: Seg[]): any {
    try {
      ...
      for (const s of segs.slice(1)) {
        v = eng.deref(v);
        if (isRec(v)) {
          const st = eng.forceState(v, s as string);
          v = st === 'ok' ? v.slots.get(s as string)!.value : undefined;
  ...
  private readSegs(eng: Engine, inst: any, rd: Expr, entry: Module): Seg[] | null {
    ...
    const sibling = (n: string): any => {
      let i = inst;
      while (i) {
        if (i.slots.has(n)) return i;
        i = i.parent;
      }
      return null;
    };
  ...
  complete(text: string, commands: string[]): string[] {
    ...
    if (text.startsWith(':')) {
      ...
      switch (cmd) {
        case ':save':
        case ':bind':
          return rest.includes('=') ? completeFile(rest.slice(rest.indexOf('=') + 1)) : by(roots());
        case ':evaluate':
        case ':validate':
        case ':unbind':
        case ':diff':
          return by(roots());
        case ':drop':
          return by([...this.state.decls.keys(), ...this.state.outputs.keys()]);
        case ':set':
        ...
        case ':update':
        case ':remove':
          return this.completePath(last);
        case ':load':
        case ':write':
        case ':history':
          return completeFile(rest);
        default:
          return [];
      }
    }
    const m =
      /([A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_][A-Za-z0-9_$]*|\[[^\]]*\])*)\.([A-Za-z_]*)$/.exec(
        text,
      );
    if (m) {
      const prefix = m[2];
      if (m[1] === 'std' || m[1].startsWith('std.')) {
        ...
        return uniq(
          Object.keys(STD)
            ...
            .map((k) => k.slice(ns.length).split('.')[0])
      ...
      try {
        const b = this.build(this.state);
        if (b.entry) {
          const cx = this.sessionCtx(this.state, b.entry.env, () => {});
          rt = infer(cx, parseExpr(m[1])).rt;
        }
      } catch {
        return [];
      }
      const members = (t: RT | null): any[] | null => {
        ...
        if (t.t === 'union') {
          const sets = t.arms.map(members);
          if (sets.some((s: any) => !s)) return null;
          return sets[0].filter((m: any) =>
            sets.every((s: any) => s.some((x: any) => x.name === m.name)),
          );
        }
      ...
      return uniq(
        ms
          .filter((x: any) => x.name.startsWith(prefix))
          .map(
            (x: any) =>
              `${x.name}${x.hidden ? '$' : ''}  ${x.kind === 'der' ? 'derived' : x.kind === 'dflt' ? 'defaulted' : x.kind === 'opt' ? 'optional' : 'required'}${x.type ? `: ${typeText(x.type)}` : ''}`,
          ),
      );
    }
    ...
    if (prefix.startsWith('$'))
      return uniq(
        ['$this', '$parent', '$root', '$key', '$path', '$referrers'].filter((x) =>
          x.startsWith(prefix),
        ),
      );
  ...
  private completePath(partial: string): string[] {
    ...
    if (!m || (!partial.includes('.') && !partial.includes('[')))
      return this.allRootNames()
  ...
  scratchText(): string {
    ...
    for (const [n, o] of this.state.outputs)
      parts.push(`output ${n}: ${o.type ?? this.inferredTypeText(o.expr)} = ${o.expr}`);
    return parts.length ? parts.join('\n') + '\n' : '';
  }
  private inferredTypeText(expr: string): string {
    try {
      return this.typeOf(expr).type;
    } catch {
      return 'any';
    }
  }
  ...
  moduleText(): string {
    ...
    const header =
      names.length && this.entryPath
        ? `import { ${names.join(', ')} } from "./${basename(this.entryPath)}"\n\n`
        : '';
    return header + body;
  }
  fmt(): string {
    const t = this.scratchText();
    return t ? format(t) : '';
  }
  write(file: string): void {
    try {
      host.writeFile(file, this.moduleText());
    } catch {
      throw new SessionError(`cannot write ${file}`);
    }
  }
  ...
  diff(name: string): string[] {
    ...
    if (!d)
      throw new SessionError(
        this.hasRoot(name) ? `${name} holds no document` : `no root named ${name}`,
      );
    const before = docJson(d.base),
      after = docJson(d.doc);
  ...
  sessionLines(): string[] {
    const out: string[] = [];
    for (const [n, t] of this.state.decls)
      out.push(`declaration  ${n.padEnd(16)} ${t.trim().split('\n')[0]}`);
    for (const [n, o] of this.state.outputs)
      out.push(`output       ${n.padEnd(16)} ${n}${o.type ? `: ${o.type}` : ''} = ${o.expr}`);
    for (const [n, d] of this.state.documents)
      out.push(
        `document     ${n.padEnd(16)} ${d.origin}${d.file ? ` ${d.file}` : ''}${d.edited ? ' (edited)' : ''}`,
      );
    return out;
  }
  historyLines(): string[] {
    const out = [`${this.cursor === 0 ? '*' : ' '} 0  (start)`];
    this.log.forEach((op, i) =>
      out.push(`${this.cursor === i + 1 ? '*' : ' '} ${i + 1}  ${opText(op)}`),
    );
    return out;
  }
  scriptLines(): string[] {
    return this.log.slice(0, this.cursor).map(opText);
  }
}
...
export function opText(op: Op): string {
  switch (op.op) {
    case 'bind':
      return op.src.kind === 'file'
        ? `:bind ${op.name}=${op.src.file}`
        : op.src.kind === 'inline'
          ? `:bind ${op.name} ${docJson(readJson(op.src.text))}`
          : `:bind ${op.name} = ${op.src.text.trim()}`;
    case 'unbind':
      return `:unbind ${op.name}`;
    case 'edit':
      return `:${op.kind} ${op.path}${op.expr !== undefined ? ` = ${op.expr.trim()}` : ''}`;
    case 'declare':
      return op.text.trim();
    case 'output':
      return `${op.name}${op.type ? `: ${op.type}` : ''} = ${op.expr.trim()}`;
    case 'drop':
      return `:drop ${op.name}`;
    case 'reload':
      return ':reload';
...
function readsOf(e: Expr): Expr[] {
  const out: Expr[] = [];
  const isChain = (x: any): boolean =>
    x.e === 'name' || x.e === 'ctx' || ((x.e === 'member' || x.e === 'index') && isChain(x.x));
  const go = (x: any) => {
    if (!x || typeof x !== 'object') return;
    if (isChain(x) && (x.e === 'member' || x.e === 'index')) {
      out.push(x);
      if (x.e === 'index') go(x.i);
      return;
    }
...
export function exprText(e: any): string {
  switch (e.e) {
    ...
    case 'ctx':
      return e.name;
    case 'member':
      return `${exprText(e.x)}${e.safe ? '?.' : '.'}${e.name}`;
    case 'index':
    ...
    case 'un':
      return `${e.op}${exprText(e.x)}`;
    case 'call':
      return `${exprText(e.fn)}(${e.args.map(exprText).join(', ')})`;
    case 'if':
      return `if ${exprText(e.c)} then ${exprText(e.t)} else ${exprText(e.f)}`;
    case 'referrers':
      return `$referrers(${e.type}, ${JSON.stringify(e.member)})`;
    case 'template':
      return (
        '`' +
        e.parts.map((p: any) => (typeof p === 'string' ? p : '${' + exprText(p) + '}')).join('') +
        '`'
      );
    case 'obj':
    ...
    case 'comp':
      return `[for ${e.clauses.map((c: any) => `${c.v} in ${exprText(c.iter)}`).join(', ')} … ]`;
    case 'lambda':
      return `(${e.params.join(', ')}) => …`;
    case 'with':
      return `${exprText(e.base)} with …`;
    case 'match':
      return `match ${exprText(e.subject)} { … }`;
    default:
      return '…';
  }
}
