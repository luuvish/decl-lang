...
export function initFormatter(): Promise<void> {
...
export function format(src: string): string {
  ...
  for (const line of lines) {
    ...
    if (
      closers === 0 &&
      (CONT_STARTERS.has(first.text) ||
        (lastCode !== null &&
          !ATOMS.has(lastCode.type) &&
          CONT_ENDERS.has(lastCode.text) &&
          !isTypeAngle(lastCode)))
    )
      // `ref<...>` closes a type, it opens nothing
      indent = depth + 1;
    let text = '    '.repeat(indent);
    let prev: Leaf | null = null,
      prev2: Leaf | null = null;
    for (const l of line) {
      if (prev) {
