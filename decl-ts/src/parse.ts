...
export function parseSource(src: string): ParseResult {
  ...
  for (const c of tree.rootNode.namedChildren) {
    ...
    if (d) {
      ...
      const start = c.previousSibling?.text === 'export' ? c.previousSibling.startPosition : c.startPosition;
...
// ---------------- types ----------------
const locOf = (n: Node): Loc => ({ sl: n.startPosition.row, sc: n.startPosition.column, el: n.endPosition.row, ec: n.endPosition.column });
function lowerType(n: Node): TypeAst { const t = lowerType0(n); t.loc = locOf(n); return t; }
...
// ---------------- members ----------------
function lowerMember(n: Node): MemberAst | null { const m = lowerMember0(n); if (m) m.loc = locOf(n); return m; }
...
function lowerExpr(n: Node): Expr { const e = lowerExpr0(n); e.loc = locOf(n); return e; }
