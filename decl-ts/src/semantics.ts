...
export function sortDiags(diags: Diag[]): Diag[] {
  const segsOf = (p: string): Seg[] => {
    try {
      return p ? parsePath(p, '') : [];
    } catch {
      return [p];
    }
  };
  return diags
    .map((d, i) => ({ d, i, segs: segsOf(d.path) }))
    .sort(
      (a, b) =>
        cmpPath(a.segs, b.segs) ||
        ((a.d.id ?? '') < (b.d.id ?? '') ? -1 : (a.d.id ?? '') > (b.d.id ?? '') ? 1 : 0) ||
        a.i - b.i,
    )
    .map((x) => x.d);
}
...
export class Env {
  ...
  funcs = new Map<
    string,
    { params: { name: string; type?: TypeAst }[]; ret?: TypeAst; body: Expr }
  >();
  duplicates: string[] = [];
  ...
  diags = new Map<
    string,
    { params: { name: string }[]; severity: string; template: TemplateParts }
  >();
  registry: RecInst[] = [];
  ...
  constructor() {
    ...
    const dim = (name: string, terms?: { name: string; exp: number }[]) =>
      this.dimDecls.set(name, { terms });
    ...
    const prefixable = [
      ...bases.map(([, s]) => s).filter((s) => s !== 'kg'),
      ...derived.map(([, , s]) => s).filter((s) => s !== 'bit'),
      'g',
    ];
    for (const u0 of prefixable)
      for (const [p, f] of PREFIXES) unit(p + u0, { factor: f, base: u0 });
  ...
  unitInfo(sym: string, visiting = new Set<string>()): { key: string; toBase: number } {
    ...
    if (u.dim !== undefined) {
      ...
      if (f === undefined && u.factor && this.exprEval) {
        try {
          f = this.exprEval(u.factor);
        } catch {
          /* an unevaluable factor stays undefined */
        }
      }
        ...
        if (u.dim !== undefined) {
          ...
          if (prev)
            out.push({
              severity: 'error',
              code: 'E4073',
              message: `second base unit ${sym} for dimension ${info.key} (base is ${prev})`,
              path: '',
            });
  ...
  load(decls: Decl[]) {
    ...
    const claim = (n: string) => {
      if (seen.has(n)) this.duplicates.push(n);
      seen.add(n);
    };
    for (const d of decls) {
      ...
      if (d.d === 'dimension') {
        if (this.dimDecls.has(d.name))
          this.spaceDiags.push({
            severity: 'error',
            code: 'E3001',
            message: `dimension ${d.name} redeclared`,
            path: '',
          });
        ...
        if (this.unitDecls.has(d.name))
          this.spaceDiags.push({
            severity: 'error',
            code: 'E4073',
            message: `unit ${d.name} redeclared`,
            path: '',
          });
        else this.unitDecls.set(d.name, { dim: d.dim, factor: d.factor, base: d.base });
      }
      if (d.d === 'type')
        this.typeAsts.set(d.name, { ast: d.type, tail: d.tail, params: (d as any).params });
      else if (d.d === 'const')
        this.consts.set(d.name, { expr: d.expr, type: d.type, state: 'unforced' });
      else if (d.d === 'func')
        this.funcs.set(d.name, { params: d.params, ret: d.ret, body: d.body });
      else if (d.d === 'output') this.outputs.push(d);
      else if (d.d === 'input') this.inputs.set(d.name, { type: d.type, fallback: d.fallback });
      else if (d.d === 'diagnostic') this.diags.set(d.name, d);
    }
  }
  ...
  constNum(v: any): any {
    ...
    const diag = (code: string, message: string) => {
      ...
      if (this.onConstDiag) this.onConstDiag(d);
      else this.report(d);
    };
    try {
      ...
      if (e instanceof EvalErr) {
        const code = /zero/.test(e.message)
          ? 'E5001'
          : /NaN|Infinity/.test(e.message)
            ? 'E5002'
            : 'E5001';
  ...
  resolve(ast: TypeAst, name?: string): RT {
    switch (ast.k) {
      ...
      case 'range': {
        const lo = this.constNum(ast.lo),
          hi = this.constNum(ast.hi);
      ...
      case 'map':
        return { t: 'map', key: this.resolve(ast.key), val: this.resolve(ast.val) };
      case 'array': {
        const lo = this.constNum(ast.lo),
          hi0 = this.constNum(ast.hi);
        const hi = ast.excl && typeof hi0 !== 'string' && hi0 !== undefined ? Number(hi0) - 1 : hi0;
        return {
          t: 'arr',
          elem: this.resolve(ast.elem),
          lo: typeof lo === 'bigint' ? Number(lo) : lo,
          hi: typeof hi === 'bigint' ? Number(hi) : hi,
        };
      }
      case 'union':
        return { t: 'union', arms: ast.arms.map((a) => this.resolve(a)) };
      case 'isect': {
  ...
  extendInto(target: any, base: RT, ext: RT) {
    const b: any = base,
      e: any = ext;
    if (b.t !== 'rec') {
      Object.assign(target, b, { filling: false });
      return;
    }
    if (b.filling) {
      (b.pendingExts ??= []).push({ target, ext });
      return;
    }
    target.open = b.open;
    ...
    for (const cd of e.ctxDecls ?? []) {
      ...
      if (i >= 0) ctxDecls[i] = cd;
      else ctxDecls.push(cd);
    }
    ...
    for (const om of e.members) {
      ...
      if (i >= 0) target.members[i] = om;
      else target.members.push(om);
    }
  ...
  expandPattern(re: string): string {
    ...
    return re.replace(/\$\{([^}]*)\}/g, (_, inner: string) => {
      ...
      const frags = arms.map((arm) => {
        ...
        if ((m = /^"((?:[^"\\]|\\.)*)"$/.exec(arm)))
          return this.patternFragment({ t: 'lit', v: JSON.parse(`"${m[1]}"`) }, text);
        if ((m = /^(-?[0-9]+)\.\.(<?)(-?[0-9]+)$/.exec(arm)))
          return this.patternFragment(
            { t: 'range', base: 'int', lo: BigInt(m[1]), hi: BigInt(m[3]), excl: m[2] === '<' },
            text,
          );
        if (/^-?[0-9]+$/.test(arm)) return this.patternFragment({ t: 'lit', v: BigInt(arm) }, text);
        if (!/^[A-Za-z_][A-Za-z0-9_.]*$/.test(arm))
          throw new Error(`pattern interpolation of ${text}: not a type (§3.6)`);
        if (visiting.has(arm)) throw new Error(`pattern interpolation of ${arm} is circular`);
        visiting.add(arm);
        let rt: RT;
        try {
          rt = this.resolve({ k: 'named', name: arm, args: [] });
        } catch (e: any) {
          visiting.delete(arm);
          if (/^unknown type/.test(e.message))
            throw new Error(`pattern interpolation of ${arm}: unknown type`, { cause: e });
          throw e;
        }
        visiting.delete(arm);
  ...
  patternFragment(rt: RT, name: string): string {
    ...
    const bad = (): never => {
      throw new Error(
        `pattern interpolation of ${name}: type is neither string- nor integer-shaped (§3.6)`,
      );
    };
    switch (rt.t) {
      case 'pattern':
      ...
      case 'lit':
        ...
        return bad();
      case 'range': {
        if (rt.base !== 'int' || typeof rt.lo !== 'bigint' || typeof rt.hi !== 'bigint')
          return bad();
        const hi = rt.excl ? rt.hi - 1n : rt.hi;
        if (hi - rt.lo >= 65536n)
          throw new Error(`pattern interpolation of ${name}: range too large (limit 65536 values)`);
        const alts: string[] = [];
        for (let v = rt.lo; v <= hi; v++) alts.push(v.toString());
        return `(?:${alts.join('|')})`;
      }
      case 'union':
        return `(?:${rt.arms.map((a: RT) => this.patternFragment(a, name)).join('|')})`;
      case 'pred':
        return this.patternFragment(rt.base, name);
      case 'prim':
        return rt.name === 'string' ? '.*' : rt.name === 'int' ? '-?[0-9]+' : bad();
      default:
        return bad();
    }
  }
  ...
  instantiate(
    ast: TypeAst & { k: 'named' },
    decl: { ast: TypeAst; tail?: ElseTail; params?: any[] },
  ): RT {
    const ps = decl.params!;
    if (ast.args.length !== ps.length)
      throw new Error(
        `generic arity: ${ast.name} expects ${ps.length} argument(s), got ${ast.args.length}`,
      );
    ...
    for (let i = 0; i < ps.length; i++) {
      ...
      if (p.type) {
        ...
        if (a.k === 'lit') v = a.v;
          ...
          throw new Error(
            `generic arity: parameter ${p.name} of ${ast.name} takes a constant value`,
          );
    ...
    switch (ast.k) {
      case 'named': {
        ...
        return {
          ...ast,
          args: ast.args.map(t),
          ext: ast.ext ? t(ast.ext) : undefined,
      ...
      case 'array': {
        const sub = (v: any) =>
          typeof v === 'string' && values.has(v) ? Number(values.get(v)) : v;
        return { ...ast, elem: t(ast.elem), lo: sub(ast.lo), hi: sub(ast.hi) };
      }
      case 'record':
        return { ...ast, members: ast.members.map((m) => this.substMember(m, types, values)) };
      case 'map':
        return { ...ast, key: t(ast.key), val: t(ast.val) };
      case 'union':
      case 'isect':
        return { ...ast, arms: ast.arms.map(t) };
      case 'func':
        return { ...ast, params: ast.params.map(t), ret: t(ast.ret) };
      default:
        return ast;
    }
  }
  substMember(m: MemberAst, types: Map<string, TypeAst>, values: Map<string, any>): MemberAst {
    ...
    switch (m.m) {
      case 'value':
        return { ...m, type: t(m.type), dflt: m.dflt ? substExpr(m.dflt, values) : undefined };
      case 'derived':
        return { ...m, type: m.type ? t(m.type) : undefined, expr: substExpr(m.expr, values) };
      case 'context':
        return { ...m, type: t(m.type) };
      case 'assert':
        return { ...m, cond: substExpr(m.cond, values) };
      case 'when':
        return {
          ...m,
          cond: substExpr(m.cond, values),
          body: m.body.map((b) => this.substMember(b, types, values)),
        };
    }
  }
  ...
  fillRecord(rt: any, members: MemberAst[]) {
    ...
    for (const m of members) {
      if (m.m === 'value')
        rt.members.push({
          kind: m.dflt ? 'dflt' : m.opt ? 'opt' : 'req',
          name: m.name,
          type: this.resolve(m.type),
          dflt: m.dflt,
          menv: this,
        });
      else if (m.m === 'derived')
        rt.members.push({
          kind: 'der',
          name: m.name,
          type: m.type ? this.resolve(m.type) : undefined,
          expr: m.expr,
          menv: this,
    ...
    for (const r of recs) {
      for (const m of r.members) {
        ...
        if (i >= 0)
          merged.members[i] = {
            ...merged.members[i],
            conj: [...(merged.members[i].conj ?? [merged.members[i].type]), m.type],
            kind: m.kind === 'req' ? 'req' : merged.members[i].kind,
          };
...
export function substExpr(e: Expr, values: Map<string, any>): Expr {
  ...
  for (const [k, v] of Object.entries(e)) {
    if (Array.isArray(v))
      out[k] = v.map((x) => (x && typeof x === 'object' ? substExpr(x, values) : x));
    else if (v && typeof v === 'object') out[k] = substExpr(v, values);
    else out[k] = v;
  }
  return out;
}
...
export function pathStr(segs: Seg[], relRoot?: string): string {
  ...
  segs.forEach((s, i) => {
    if (i === 0) {
      out += relRoot !== undefined && s === relRoot ? '$' : String(segText(s));
      return;
    }
...
export function parsePath(s: string, rootName: string): Seg[] {
  ...
  if (s[0] === '$') {
    ...
    const m = /^[_A-Za-z][_A-Za-z0-9]*/.exec(s);
...
export function valueEq(a: any, b: any): boolean {
  ...
  if (isArr(a) && isArr(b))
    return (
      ...
      a.items.every((x: any, i: number) => valueEq(x, b.items[i]))
    );
  if (isMap(a) && isMap(b)) {
    if (a.entries.size !== b.entries.size) return false;
    for (const [k, v] of a.entries) {
      if (!b.entries.has(k) || !valueEq(v, b.entries.get(k))) return false;
    }
    return true;
  }
  if (isRec(a) && isRec(b)) {
...
export function readJson(src: string): any {
  ...
  function val(): any {
    ...
    if (c === '{') {
      i++;
      const o: any = { __jobj: true, entries: [] as [string, any][] };
      ws();
      if (src[i] === '}') {
        i++;
        return o;
        ...
        s +=
          e === 'n'
            ...
            : e === 't'
              ? '\t'
              : e === 'u'
                ? String.fromCharCode(parseInt(src.slice(j + 2, j + 6), 16))
                : e;
        j += e === 'u' ? 6 : 2;
      } else {
        s += src[j];
