...
const run = (args: string[], opts: { cwd?: string; input?: string } = {}) => {
  const r = spawnSync('node', [cli, 'repl', ...args], {
    encoding: 'utf8',
    cwd: opts.cwd ?? root,
    input: opts.input,
  });
...
function firstDiff(a: string, b: string): string {
  ...
  for (let i = 0; i < Math.max(al.length, bl.length); i++)
    if (al[i] !== bl[i])
      return `line ${i + 1}: expected ${JSON.stringify(al[i])}, got ${JSON.stringify(bl[i])}`;
  return '';
}

console.log(
  '== repl: the incremental step is observationally identical to a full recomputation ==',
);
for (const c of readdirSync(join(root, 'tests/repl')).sort()) {
  const dir = join(root, 'tests/repl', c);
  if (!existsSync(join(dir, 'session.txt'))) continue;
  ...
  const full = spawnSync(
    'node',
    [cli, 'repl', ...entry, '--script', `tests/repl/${c}/session.txt`],
    { encoding: 'utf8', cwd: root, env: { ...process.env, DECL_FULL_RECOMPUTE: '1' } },
  );
  check(
    `${c}: incremental == full`,
    inc.out === full.stdout && inc.code === (full.status ?? -1),
    firstDiff(full.stdout, inc.out),
  );
}
...
console.log('== repl: files, the clock, and the command line ==');
{
  ...
  for (const f of ['main.decl', 'doc.json'])
    copyFileSync(join(root, 'tests/repl/documents', f), join(dir, f));
  const session =
    [
  ...
  check(
    ':save writes the edited document',
    readFileSync(join(dir, 'saved.json'), 'utf8') === '{"port":9100,"name":"doc"}\n',
  );
  check(
    ':write writes the scratch module',
    readFileSync(join(dir, 'scratch.decl'), 'utf8') === 'output y: 2..65536 = deployed.port + 1\n',
    readFileSync(join(dir, 'scratch.decl'), 'utf8'),
  );
  check(
    ':history file writes a replayable session',
    readFileSync(join(dir, 'log.txt'), 'utf8') ===
      ':bind deployed=doc.json\ny = deployed.port + 1\n:update deployed.port = 9100\n',
  );
  check(
    ':time reports milliseconds and the incremental step',
    /^total \d+\.\d ms \(load \d+\.\d ms, check \d+\.\d ms, bind \d+\.\d ms, evaluate \d+\.\d ms\), recomputed \d+ of \d+ slots$/m.test(
      r.out,
    ),
    r.out,
  );
