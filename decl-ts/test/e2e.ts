...
function pipeline(file: string, binds: { input: string; json: string; as?: string }[] = []) {
  ...
  for (const o of env.outputs) {
    ...
    try {
      ...
      /* a failed root is reported through the diagnostics */
    }
  }
  for (const b of binds) {
    ...
    try {
      env.roots.set(name, eng.bind(readJson(b.json), env.resolve(decl.type), [name], null, sc));
    } catch {
      /* a failed root is reported through the diagnostics */
    }
  }
  ...
  eng.validateAll('');
  env.diagnostics.sort((a, b) =>
    a.path < b.path ? -1 : a.path > b.path ? 1 : (a.id ?? '') < (b.id ?? '') ? -1 : 1,
  );
  return { env, eng };
}
...
console.log('== case 3: fixtures ==');
{
  ...
  for (const d of env.diagnostics)
    console.log('  diag:', d.severity, d.id ?? d.code, d.path, '-', d.message);
...
console.log('== case 2: config ==');
{
  ...
  check(
    'prod layering',
    get(eng, ['prod', 'host']) === 'api.internal' && get(eng, ['prod', 'workers']) === 32n,
  );
  ...
  check(
    ...
    errs.length === 2 && warns.length === 1,
    JSON.stringify(r.env.diagnostics),
  );
  check(
    'cert assert id',
    errs.some((d) => d.id === 'TlsConfig.cert_present'),
  );
...
console.log('== case 1: interconnect ==');
{
  ...
  check(
    ...
    get(eng, ['xbar', 'nodes', 'dom0', 'nodes', 'mst0', 'ports', 'si', 'width']) === 64n,
  );
  check(
    'arbiter max',
    get(eng, ['xbar', 'nodes', 'dom0', 'nodes', 'arb0', 'ports', 'mi', 'width']) === 64n,
  );
  ...
  env2.roots.set(
    'doc',
    eng2.bind(readJson(ser), env2.resolve(env2.inputs.get('doc')!.type), ['doc'], null, sc2),
  );
  eng2.forceAll(env2.roots.get('doc'), false);
  eng2.phase = 2;
  for (let i = 0; i < eng2.deferredSlots.length; i++)
    eng2.forceSlotSafe(eng2.deferredSlots[i].inst, eng2.deferredSlots[i].name);
  eng2.forceAll(env2.roots.get('doc'), true);
  eng2.validateAll('');
  for (const d of env2.diagnostics)
    console.log('  rt diag:', d.severity, d.id ?? d.code, d.path, '-', d.message);
  ...
  const env3 = new Env();
  env3.load(parseSource(src).decls);
  const eng3 = new Engine(env3);
  const sc3 = { inst: null, locals: new Map<string, any>(), rootName: 'doc' };
  env3.roots.set(
    'doc',
    eng3.bind(doc, env3.resolve(env3.inputs.get('doc')!.type), ['doc'], null, sc3),
  );
  eng3.forceAll(env3.roots.get('doc'), false);
  eng3.phase = 2;
  for (let i = 0; i < eng3.deferredSlots.length; i++)
    eng3.forceSlotSafe(eng3.deferredSlots[i].inst, eng3.deferredSlots[i].name);
  eng3.forceAll(env3.roots.get('doc'), true);
  eng3.validateAll('');
  const errs = env3.diagnostics.filter((d) => d.severity === 'error');
  check(
    'one width_match at root cause',
    errs.length === 1 && errs[0].id === 'Edge.width_match',
    JSON.stringify(errs),
  );
}
...
console.log('== guide: end to end ==');
{
  ...
  eng.validateAll('');
  check(
    'guide evaluates clean',
    env.diagnostics.length === 0,
    JSON.stringify(env.diagnostics.slice(0, 3)),
  );
  ...
  check('guide quantity default', get(eng, ['demo', 'services', 0, 'timeout']).value === 0.25);
...
{
  for (const f of [
    'docs/examples/01_interconnect.decl',
    'docs/examples/02_config.decl',
    'docs/examples/03_fixtures.decl',
  ]) {
    const { decls, errors } = parseSource(readFileSync(join(root, f), 'utf8'));
    const checks = errors.length ? [] : checkModule(decls);
    check(
      `${f.split('/').pop()} check-clean`,
      errors.length === 0 && checks.length === 0,
      JSON.stringify(checks.slice(0, 3)),
    );
  }
  const md = readFileSync(join(root, 'docs/guide/01_overview_by_example.md'), 'utf8');
...
`;
  ...
  check(
    'match module checks clean',
    checkModule(decls).length === 0,
    JSON.stringify(checkModule(decls).slice(0, 3)),
  );
  const env = new Env();
  env.load(decls);
  const eng = new Engine(env);
  ...
  env.roots.set(
    'shape',
    eng.bind(
      readJson('{"kind":"rect","w":2.0,"h":3.0}'),
      env.resolve(env.inputs.get('shape')!.type),
      ['shape'],
      null,
      scS,
    ),
  ...
  env.roots.set(
    'proto',
    eng.bind(readJson('"grpc"'), env.resolve(env.inputs.get('proto')!.type), ['proto'], null, scP),
  );
  for (const o of env.outputs) {
    const sc = { inst: null, locals: new Map<string, any>(), rootName: o.name };
    env.roots.set(o.name, eng.bind(eng.ev(o.expr, sc), env.resolve(o.type), [o.name], null, sc));
  ...
  const env2 = new Env();
  env2.load(parseSource(src2).decls);
  const eng2 = new Engine(env2);
  ...
  env2.roots.set(
    ...
    eng2.bind(
      readJson('"tcp"'),
      env2.resolve(env2.inputs.get('proto')!.type),
      ['proto'],
      null,
      sc2,
    ),
...
`;
  ...
  eng.validateAll('');
  check(
    'generic outputs bind clean',
    env.diagnostics.length === 0,
    JSON.stringify(env.diagnostics.slice(0, 2)),
  );
  ...
  const env2 = new Env();
  env2.load(parseSource(src).decls);
  const eng2 = new Engine(env2);
  ...
  try {
    eng2.bind(
      eng2.ev(
        { e: 'arr', items: [1n, 2n].map((v) => ({ spread: false, expr: { e: 'lit', v } })) } as any,
        sc2,
      ),
      env2.resolve({
        k: 'named',
  ...
  check(
    'size violation surfaces',
    env2.diagnostics.length > mark,
    JSON.stringify(env2.diagnostics),
  );
}
...
`;
  ...
  eng.validateAll('');
  check(
    'quantity module binds clean',
    env.diagnostics.length === 0,
    JSON.stringify(env.diagnostics.slice(0, 2)),
  );
  const speed = get(eng, ['trip', 'speed']);
  ...
  check(
    'derived-unit input normalized to base',
    ser.includes('"dist":{"value":3000.0,"unit":"m"}'),
    ser,
  );
  ...
  const inst = eng2.bind(
    doc,
    env2.resolve({ k: 'named', name: 'Trip', args: [] } as any),
    ['t'],
    null,
    sc2,
  );
  ...
  check(
    'interchange km converts to base',
    (eng2.resolveSegs as any) && inst.slots.get('dist')!.value.value === 2000,
  );
}
