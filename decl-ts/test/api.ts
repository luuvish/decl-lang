...
const caught = async (f: () => Promise<unknown>): Promise<DeclError | null> => {
  try { await f(); return null; } catch (e) { return e instanceof DeclError ? e : (() => { throw e; })(); }
};
...
console.log('== evaluate ==');
{
  ...
  ok('default: the exported outputs by name', Object.keys(all).join(',') === 'base,prod,dev', Object.keys(all).join(','));
  const one = await evaluate(cfg, { outputs: ['prod'] });
  ok('outputs selects roots', Object.keys(one).join(',') === 'prod' && (one.prod as any).host === 'api.internal', JSON.stringify(one).slice(0, 80));
  ...
  ok('an input bound from a file is a root', JSON.stringify(byFile.base) === '{"host":"h","port":8}' && JSON.stringify(byFile.copy) === '{"host":"h","port":8}', JSON.stringify(byFile));
  const byValue = await evaluate(fb, { inputs: { base: { host: 'v' } }, outputs: ['copy'] });
  ok('an input bound from a value completes', JSON.stringify(byValue.copy) === '{"host":"v","port":80}', JSON.stringify(byValue));
  ...
  ok('an unreadable document is E6004', e3?.diagnostics[0]?.code === 'E6004', JSON.stringify(e3?.diagnostics));
  const e4 = await caught(() => evaluate(cfg, { inputs: { deployed: { host: 'x', port: 70000, workers: 100, tls: { enabled: true } } }, outputs: ['deployed'] }));
  ok('error diagnostics come back on the DeclError', !!e4 && e4.diagnostics.some(d => d.severity === 'error') && e4.message === e4.diagnostics[0].message, JSON.stringify(e4?.diagnostics.slice(0, 2)));
}
...
console.log('== check / validate / formatSource ==');
{
  ...
  ok('check: static error reported with file, code, path', bad.length > 0 && bad[0].code === 'E4011' && Object.keys(bad[0])[0] === 'file', JSON.stringify(bad));
  const cfg = join(root, 'docs/examples/02_config.decl');
  const v = await validate(cfg, { inputs: { deployed: { host: 'x', port: 70000, workers: 100, tls: { enabled: true } } } });
  ok('validate: diagnostics of a bound document', v.some(d => d.severity === 'error'), JSON.stringify(v.slice(0, 2)));
  const e = await caught(() => validate(join(root, 'tests/validation/lexical/invalid/semicolon.decl')));
