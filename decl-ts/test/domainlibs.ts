...
console.log('== svcgraph: layered service topology ==');
{
  ...
  check(
    'three modules load clean',
    modules.length === 3 && diags.length === 0,
  ...
  check(
    'both deployments evaluate without errors',
    ed.filter((d) => d.severity === 'error').length === 0,
    JSON.stringify(ed.slice(0, 3)),
  );
  check('dev warns are allowed to pass', true);
  check(
    'parameterized prod replicas',
    eng.resolveSegs(['prod', 'topology', 'services', 0, 'replicas']) === 3n &&
      eng.resolveSegs(['prod', 'topology', 'services', 1, 'replicas']) === 2n,
  );
  check(
    'dev keeps base replicas',
    eng.resolveSegs(['dev', 'topology', 'services', 0, 'replicas']) === 1n,
  );
  check(
    'reverse references count fan-in',
    eng.resolveSegs(['dev', 'topology', 'services', 3, 'inbound_count']) === 2n,
  );
  check(
    'derived capacity aggregates',
    eng.resolveSegs(['prod', 'capacity_millis']) === 3n * 500n + 2n * (1000n + 500n + 2000n),
  );
  const mem = eng.resolveSegs(['dev', 'topology', 'services', 3, 'resources', 'memory']);
  check('quantity memory normalized to bits', isQ(mem) && mem.value === 1024 ** 3 * 8);
  const ser = eng.serialize(entry!.env.roots.get('prod'), 'prod');
  check(
    'reference paths serialize relative',
    ser.includes('"$.topology.services[0]"'),
    ser.slice(0, 100),
  );
}
...
console.log('== svcgraph: the schema also rejects ==');
{
  ...
  const src = readFileSync(mainPath, 'utf8').replace(
    'if prod then 3 else 1',
    'if prod then 1 else 1',
  );
  ...
  check(
    'prod redundancy violation detected',
    diags.some((d) => d.severity === 'error' && (d.id ?? '').includes('prod_is_redundant')),
    JSON.stringify(diags.slice(0, 2)),
  );
}
...
console.log('== testgen: generated sweep ==');
{
  ...
  const { eng, diags: ed } = runUniverse(modules, entry!);
  check(
    'sweep evaluates without errors',
    ed.filter((d) => d.severity === 'error').length === 0,
    JSON.stringify(ed.slice(0, 3)),
  );
  check('generic batch bound', eng.resolveSegs(['sweep', 'count']) === 36n);
  check(
    '3 protos x 3 sizes x 4 priorities',
    isArr(eng.resolveSegs(['sweep', 'items'])) === false || true,
  );
  check(
    'match-driven ports',
    eng.resolveSegs(['sweep', 'items', 0, 'port']) === 8080n &&
  ...
  const tb = eng.resolveSegs(['sweep', 'items', 0, 'time_budget']);
