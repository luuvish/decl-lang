...
function validate(doc: any): { errs: any[]; env: Env; eng: Engine } {
  ...
  try {
    env.roots.set(
      'site',
      eng.bind(
        readJson(JSON.stringify(doc)),
        env.resolve({ k: 'named', name: 'Fabric', args: [] } as any),
        ['site'],
        null,
        sc,
      ),
...
console.log('== synthetic sites validate ==');
{
  ...
  const speed = eng.resolveSegs(['site', 'ports', 'wan0', 'params', 'speed']);
  check(
    'bandwidth normalizes to its base unit',
    speed && speed.dim === 'DataSize*Time^-1' && speed.value === 4e11,
  );
  const ser1 = eng.serialize(env.roots.get('site'), 'site');
  const { errs: e2, eng: eng2, env: env2 } = validate(JSON.parse(ser1));
  check(
    'serialized site re-validates (round trip)',
    e2.length === 0,
    JSON.stringify(e2.slice(0, 2)),
  );
  ...
  for (const [s, l] of [
    ...
    check(
      `${s}x${l} site (${s * l} links) validates clean in ${Date.now() - t0}ms`,
      errs.length === 0,
...
console.log('== corruption is detected ==');
{
  const cases: [string, (d: any) => void, string][] = [
    ...
    [
      'zero line rate',
      (d) => {
        d.ports.wan0.params.speed.value = 0;
      },
      'eth_has_speed',
    ],
    [
      ...
      (d) => {
        d.edges.sl0x0.endpoints = ['spine0', 'ghost9'];
      },
    ...
    [
      'dangling uplink',
      (d) => {
        d.params.uplink = 'no_such_port';
      },
      'uplink_exists',
    ],
    [
      'duplicate subnet vlan',
      (d) => {
        d.params.subnets[1].vlan = 100;
      },
      'subnet_vlans_distinct',
    ],
    [
      'edge key mismatch',
      (d) => {
        d.edges.sl0x0.name = 'other';
      },
      'edge_keys_match',
    ],
  ];
  for (const [name, corrupt, marker] of cases) {
    ...
    check(
      `corruption detected: ${name}`,
      errs.length > 0 && errs.some((e) => `${e.id ?? ''}${e.message}${e.path}`.includes(marker)),
      JSON.stringify(errs.slice(0, 2)),
    );
  }
}
