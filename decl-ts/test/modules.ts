...
console.log('== basic: named / renamed / namespace imports, re-export ==');
{
  ...
  check(
    'three modules load, no diagnostics',
  ...
  const vel = entry!.env.roots.get('vel');
...
console.log('== module-graph errors ==');
{
  ...
  check(
    'import cycle is E3007',
    cyc.diags.some((d) => d.code === 'E3007'),
    JSON.stringify(cyc.diags),
  );

  const ne = loadModules(join(root, 'tests/modules/errors/not_exported.decl'));
  check(
    'unexported name is E3005',
    ne.diags.some((d) => d.code === 'E3005'),
    JSON.stringify(ne.diags),
  );

  const col = loadModules(join(root, 'tests/modules/errors/collision.decl'));
  check(
    'import collision is E3006',
    col.diags.some((d) => d.code === 'E3006'),
    JSON.stringify(col.diags),
  );

  const rc = loadModules(join(root, 'tests/modules/errors/root_a.decl'));
  check(
    'root-name clash is E3018',
    rc.diags.some((d) => d.code === 'E3018'),
    JSON.stringify(rc.diags),
  );

  const nf = loadModules(join(root, 'tests/modules/errors/missing_target.decl'));
  check(
    'missing module is E3004',
    nf.diags.some((d) => d.code === 'E3004'),
    JSON.stringify(nf.diags),
  );
}
