...

{
  ...
  check('code action: import the unknown name from the module beside', ca.some((x: any) => x.title === 'import cap from "./lib.decl"'), JSON.stringify(ca));
  // linked editing and rename of a local variable; the member-kind conversions; flipping a comparison
  const actionsSrc = 'type Pair = {\n    a: int,\n    b?: int,\n    c = a + 1\n}\nconst xs = [x * 2 for x in [1, 2] if x > 0]\nconst cmp = 3 < 4\n';
  p = nextDiagnostics(mainUri);
  notifyServer('textDocument/didChange', { textDocument: { uri: mainUri, version: 50 }, contentChanges: [{ text: actionsSrc }] });
  await p;
  const le = await request('textDocument/linkedEditingRange', { textDocument: { uri: mainUri }, position: { line: 5, character: 12 } });
  check('linked editing of a comprehension variable', le && le.ranges.length === 3, JSON.stringify(le));
  const lr = await request('textDocument/rename', { textDocument: { uri: mainUri }, position: { line: 5, character: 12 }, newName: 'y' });
  check('rename of a local variable', lr && lr.changes[mainUri].length === 3, JSON.stringify(lr));
  const conv = await request('textDocument/codeAction', { textDocument: { uri: mainUri }, range: { start: { line: 3, character: 4 }, end: { line: 3, character: 4 } }, context: { diagnostics: [] } });
  check('assists on a derived member: annotate, hide, export', ['annotate: int', 'make hidden (x$)', 'export Pair'].every(t => conv.some((x: any) => x.title === t)), JSON.stringify(conv.map((x: any) => x.title)));
  const flip = await request('textDocument/codeAction', { textDocument: { uri: mainUri }, range: { start: { line: 6, character: 14 }, end: { line: 6, character: 14 } }, context: { diagnostics: [] } });
  check('assist: flip the comparison', flip.some((x: any) => x.title === 'flip the comparison' && x.edit.changes[mainUri][0].newText === '4 > 3'), JSON.stringify(flip));
  ...
  const src2 = 'type Parent = { name: string, port: 1..65535 }\ntype Child = Parent { port: int }\ntype Item = { label = $parent.name }\ntype A = { a: int }\ntype B = { b: int }\ntype U = A | B\nconst K = 2\nconst twice = K + K\ntype W = { inner: { q: int } }\nconst dur = 250ms\ntype R = {\n    d = 1,\n    a: int\n}\ntype Ctx = { $parent: ref<Parent>, tag = $parent.name }\n';
  p = nextDiagnostics(mainUri);
  notifyServer('textDocument/didChange', { textDocument: { uri: mainUri, version: 60 }, contentChanges: [{ text: src2 }] });
  const d2 = await p;
  const at = (line: number, ch: number, diags: any[] = d2.diagnostics) => request('textDocument/codeAction', { textDocument: { uri: mainUri }, range: { start: { line, character: ch }, end: { line, character: ch } }, context: { diagnostics: diags } });
  const titles = (xs: any[]) => xs.map((x: any) => x.title);
  check('fix: the parent\'s declaration', titles(await at(1, 22)).includes("use the parent's declaration: port: 1..65535"), JSON.stringify(titles(await at(1, 22))));
  check('fix: declare the context variable', titles(await at(2, 24)).includes('declare $parent: ref<{ ... }> on Item'), JSON.stringify(titles(await at(2, 24))));
  check('fix: the discriminant', titles(await at(5, 5)).includes('add a discriminant `kind` to the arms of U'), JSON.stringify(titles(await at(5, 5))));
  const inl = await at(6, 0, []);
  check('assist: inline the constant', inl.some((x: any) => x.title === 'inline K' && x.edit.changes[mainUri].length === 3), JSON.stringify(titles(inl)));
  check('assist: extract the inline record type', titles(await at(8, 11, [])).includes('extract to a named type'), JSON.stringify(titles(await at(8, 11, []))));
  check('assist: the unit in its base unit', titles(await at(9, 12, [])).includes('convert to 0.25s'), JSON.stringify(titles(await at(9, 12, []))));
  const ro = await at(10, 5, []);
  check('assist: reorder the members', ro.some((x: any) => x.title === 'reorder the members canonically' && x.edit.changes[mainUri][0].newText === '{\n    a: int\n    d = 1\n}'), JSON.stringify(ro.filter((x: any) => x.title.startsWith('reorder'))));
  notifyServer('workspace/didChangeConfiguration', { settings: { decl: { inlayHints: { contextVariables: true } } } });
  await nextDiagnostics(mainUri);
  const cvh = await request('textDocument/inlayHint', { textDocument: { uri: mainUri }, range: { start: { line: 0, character: 0 }, end: { line: 20, character: 0 } } });
