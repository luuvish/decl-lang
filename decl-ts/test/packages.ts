...
console.log('== cross-package imports under exact pins ==');
{
  ...
  check(
    'closed set resolved',
    u.packages.size === 1 && u.packages.get('corelib')!.version === '1.0.0',
  );
  const { modules, entry: em, diags } = loadModules(entry, u.resolver);
  check(
    'modules load across packages',
    modules.length === 2 && diags.length === 0,
  ...
  check(
    ...
    JSON.stringify(ed.slice(0, 2)),
  );
  check(
    'imported const and defaults',
    em!.env.roots.get('w') === 16n && eng.resolveSegs(['box', 'width']) === 8n,
  );
}
...
console.log('== lock file: reproducibility, fail-closed drift ==');
{
  ...
  try {
    ...
    check(
      ...
      lockText(u1) === lockText(u2) && lockText(u1).includes('corelib 1.0.0 '),
    );
    ...
    try {
      ...
      check(
        ...
        verifyLock(u3).some((d) => d.code === 'E3017'),
        JSON.stringify(verifyLock(u3)),
      );
    } finally {
    ...
    check(
      ...
      verifyLock(openPackageUniverse(entry)!).some((d) => d.code === 'E3016'),
    );
    writeFileSync(lockPath, '');
    check(
      'missing entry is E3015',
      verifyLock(openPackageUniverse(entry)!).some((d) => d.code === 'E3015'),
    );
  } finally {
...
console.log('== manifest and resolution errors ==');
{
  ...
  check(
    ...
    bad.diags.some((d) => d.code === 'E3011'),
    JSON.stringify(bad.diags),
  );
  check(
    'range pin is E3012',
    bad.diags.some((d) => d.code === 'E3012'),
  );
  ...
  check(
    ...
    con.diags.some((d) => d.code === 'E3014'),
    JSON.stringify(con.diags),
  );
}
