...
pub const STD: &[(&str, usize, StdRet)] = &[
    ...
    ("array.sort", 1, StdRet::Unknown),
    ("array.sort_by", 2, StdRet::Unknown),
    ("array.unique", 1, StdRet::Unknown),
    ("array.reverse", 1, StdRet::Unknown),
...
pub fn check_expr(cx: &Ctx, e: &Rc<Expr>, expected: Option<&RT>) -> Ty {
    ...
    if let Some(rt) = &t.rt {
        if !subsumes(&cx.env, rt, expected)
            && !deferrable(rt, expected)
            && !(cx.binding && deferrable_object(&cx.env, rt, expected))
        {
            cx.report(
                "E4001",
...
fn deferrable_object(env: &Rc<Env>, s: &RT, t: &RT) -> bool {
    match &t.k {
        RTk::Pred { base, .. } => return deferrable_object(env, s, base),
        RTk::Union(arms) => return arms.borrow().iter().any(|a| deferrable_object(env, s, a)),
        _ => {}
    }
    if let RTk::Union(arms) = &s.k {
        return arms.borrow().iter().any(|a| deferrable_object(env, a, t));
    }
    let fits =
        |a: &RT, b: &RT| subsumes(env, a, b) || deferrable(a, b) || deferrable_object(env, a, b);
    match (&s.k, &t.k) {
        (RTk::Map { .. }, RTk::Rec(_)) => true,
        (RTk::Map { val: sv, .. }, RTk::Map { val: tv, .. }) => fits(sv, tv),
        (RTk::Arr { elem: se, .. }, RTk::Arr { elem: te, .. }) => fits(se, te),
        _ => false,
    }
}
