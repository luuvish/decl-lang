...
pub fn check_module(
    ...
    let pos: Rc<RefCell<Option<Rc<Expr>>>> = Rc::new(RefCell::new(None));
    let cur_decl: Rc<Cell<Option<Loc>>> = Rc::new(Cell::new(None));
    let report: Report = {
        let out = out.clone();
        let pos = pos.clone();
        let cur = cur_decl.clone();
        Rc::new(move |code: &str, message: String| {
            let loc = pos.borrow().as_ref().and_then(expr_loc).or(cur.get());
    ...
    *env.const_diag_sink.borrow_mut() = Some(out.clone()); // constant-evaluation errors surface here
    ...
    impl<'a> Walk<'a> {
        fn walk_type(&self, t: Option<&TypeAst>, depth: i32, decl_name: Option<&str>) {
            ...
            match t {
                ...
                TypeAst::Prim { .. } | TypeAst::Lit { .. } | TypeAst::Pattern { .. } => {}
            }
        }
    ...
    let mut cx0 = make_ctx(env.clone(), report.clone());
    cx0.pos = pos.clone();
    if let Some(h) = hooks {
        cx0.record = h.record.clone();
        cx0.resolve_hook = h.resolve_hook.clone();
    }
    let cx0 = cx0;
