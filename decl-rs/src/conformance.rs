...
pub fn judge_fixture(file: &Path, is_valid: bool) -> Verdict {
    ...
    let hit = |ds: &[Diag]| {
        ds.iter().any(|d| d.code.as_deref() == Some(want.as_str()))
            && (want_msg.is_empty() || ds.iter().any(|d| d.message.contains(&want_msg)))
    };
    let verdict = |ok: bool, detail: String| Verdict {
    ...
    match phase.as_deref() {
        Some("parsing") => verdict(
            !parsed.errors.is_empty(),
            "expected parse errors, got none".into(),
        ),
        Some("checking") => {
            let checks = if parsed.errors.is_empty() {
                check_module(&parsed.decls, None, None)
            } else {
                vec![]
            };
        ...
        Some("binding") => {
            let diags = if parsed.errors.is_empty() {
                run_pipeline(&parsed.decls).diags
            } else {
                vec![]
            };
...
pub fn judge_corpus(dir: &Path) -> Vec<Verdict> {
    ...
    files
        .iter()
        .map(|f| judge_fixture(f, f.to_string_lossy().contains("/valid/")))
        .collect()
}
