...
#[derive(Debug, Clone)]
pub enum TypeAst {
    Prim { name: String, loc: Option<Loc> },
    Lit { v: Value, loc: Option<Loc> },
    Range { lo: Value, hi: Value, excl: bool, loc: Option<Loc> },
    Pattern { re: String, loc: Option<Loc> },
    Record { members: Vec<MemberAst>, open: bool, loc: Option<Loc> },
    Map { key: Box<TypeAst>, val: Box<TypeAst>, loc: Option<Loc> },
    Array { elem: Box<TypeAst>, lo: Option<Value>, hi: Option<Value>, excl: bool, loc: Option<Loc> },
    Union { arms: Vec<TypeAst>, loc: Option<Loc> },
    Isect { arms: Vec<TypeAst>, loc: Option<Loc> },
    Func { params: Vec<TypeAst>, ret: Box<TypeAst>, loc: Option<Loc> },
    Named { name: String, args: Vec<TypeAst>, preds: Option<Vec<Rc<Expr>>>, ext: Option<Box<TypeAst>>, loc: Option<Loc> },
}
impl TypeAst {
    pub fn loc(&self) -> Option<Loc> {
        match self {
            TypeAst::Prim { loc, .. } | TypeAst::Lit { loc, .. } | TypeAst::Range { loc, .. } | TypeAst::Pattern { loc, .. }
            | TypeAst::Record { loc, .. } | TypeAst::Map { loc, .. } | TypeAst::Array { loc, .. } | TypeAst::Union { loc, .. }
            | TypeAst::Isect { loc, .. } | TypeAst::Func { loc, .. } | TypeAst::Named { loc, .. } => *loc,
        }
    }
    pub fn set_loc(&mut self, l: Loc) {
        match self {
            TypeAst::Prim { loc, .. } | TypeAst::Lit { loc, .. } | TypeAst::Range { loc, .. } | TypeAst::Pattern { loc, .. }
            | TypeAst::Record { loc, .. } | TypeAst::Map { loc, .. } | TypeAst::Array { loc, .. } | TypeAst::Union { loc, .. }
            | TypeAst::Isect { loc, .. } | TypeAst::Func { loc, .. } | TypeAst::Named { loc, .. } => *loc = Some(l),
        }
    }
}
...
#[derive(Debug, Clone)]
pub enum MemberAst {
    Value { name: String, opt: bool, ty: TypeAst, dflt: Option<Rc<Expr>>, loc: Option<Loc> },
    /// `hidden`: `x$ = e` — computed for the schema's own use, never part of the value (D34)
    Derived { name: String, ty: Option<TypeAst>, expr: Rc<Expr>, hidden: bool, loc: Option<Loc> },
    Context { variable: String, ty: TypeAst, loc: Option<Loc> },
    Assert { name: String, cond: Rc<Expr>, tail: Option<Tail>, loc: Option<Loc> },
    When { cond: Rc<Expr>, body: Vec<MemberAst>, loc: Option<Loc> },
}
impl MemberAst {
    pub fn loc(&self) -> Option<Loc> {
        match self {
            MemberAst::Value { loc, .. } | MemberAst::Derived { loc, .. } | MemberAst::Context { loc, .. } | MemberAst::Assert { loc, .. } | MemberAst::When { loc, .. } => *loc,
        }
    }
    pub fn set_loc(&mut self, l: Loc) {
        match self {
            MemberAst::Value { loc, .. } | MemberAst::Derived { loc, .. } | MemberAst::Context { loc, .. } | MemberAst::Assert { loc, .. } | MemberAst::When { loc, .. } => *loc = Some(l),
        }
    }
    /// the member's name (`name` in the reference: value, derived, and assert members)
    pub fn name(&self) -> Option<&str> {
        match self {
            MemberAst::Value { name, .. } | MemberAst::Derived { name, .. } | MemberAst::Assert { name, .. } => Some(name),
            _ => None,
        }
    }
}
