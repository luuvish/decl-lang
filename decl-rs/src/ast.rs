...
pub(crate) fn walk_expr_tree(e: &Rc<Expr>, into_types: bool, f: &mut dyn FnMut(&Rc<Expr>)) {
    f(e);
...
pub(crate) fn walk_type_exprs(t: &TypeAst, into_types: bool, f: &mut dyn FnMut(&Rc<Expr>)) {
    match t {
        TypeAst::Prim { .. }
        | TypeAst::Lit { .. }
        | TypeAst::Range { .. }
