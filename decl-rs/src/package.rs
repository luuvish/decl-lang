...
const METADATA: [&str; 5] = ["description", "license", "authors", "repository", "keywords"];
...
pub fn parse_manifest(path: &Path, report: &mut dyn FnMut(&str, String)) -> Option<Manifest> {
    ...
    for line0 in src.split('\n') {
        ...
        if let Some(sec) = sec_re.captures(&line) {
            ...
            if s != "dependencies" {
                report("E3011", format!("manifest {shown}: unknown section [{s}] (fail-closed, D28)"));
                ok = false;
            }
            section = Some(s);
            continue;
        ...
        let Some(kv) = kv_re.captures(&line) else {
            report("E3011", format!("manifest {shown}: unparseable line \"{line}\""));
            ok = false;
            continue;
        };
        ...
        match section.as_deref() {
            Some("dependencies") => {
                if !name_re.is_match(&key) {
                    report("E3013", format!("manifest {shown}: invalid package name {key}"));
                    ok = false;
                    continue;
                }
                match &value {
            ...
            None => {
                if !SEMANTIC.contains(&key.as_str()) && !METADATA.contains(&key.as_str()) {
                    report("E3011", format!("manifest {shown}: unknown field {key} (fail-closed, D28)"));
                    ok = false;
                    continue;
                }
                if let Some(v) = value {
    ...
    let field = |k: &str| fields.iter().rev().find(|(n, _)| n == k).map(|(_, v)| v.clone()).unwrap_or_default();
    let (name, version) = (field("name"), field("version"));
    if !name_re.is_match(&name) {
        report("E3013", format!("manifest {shown}: invalid package name {}", json_str(&name)));
        ok = false;
    }
    if !version_re.is_match(&version) {
        report("E3012", format!("manifest {shown}: version {} is not an exact triple", json_str(&version)));
        ok = false;
    }
    if ok { Some(Manifest { name, version, dependencies: deps }) } else { None }
}
...
pub fn package_hash(dir: &Path) -> String {
    ...
    for f in files {
        let rel = f.strip_prefix(dir).unwrap_or(&f).components().map(|c| c.as_os_str().to_string_lossy().to_string()).collect::<Vec<_>>().join("/");
...
pub fn open_package_universe(entry_file: &Path) -> Option<PackageUniverse> {
    ...
    let mut report = |code: &str, message: String| diags.push(Diag::error(message, String::new(), Some(code)));
    let Some(manifest) = parse_manifest(&root_dir.join("decl.toml"), &mut report) else {
        return Some(PackageUniverse {
            root_dir,
            manifest: Manifest { name: "?".into(), version: "0.0.0".into(), dependencies: vec![] },
            packages: vec![],
    ...
    fn visit(m: &Manifest, root_dir: &Path, packages: &mut Vec<ResolvedPackage>, required_by: &mut Vec<(String, String, String)>, report: &mut dyn FnMut(&str, String)) {
        for (dep, ver) in &m.dependencies {
            if let Some((_, pv, pby)) = required_by.iter().find(|(d, _, _)| d == dep) {
                if pv != ver {
                    report("E3014", format!("package {dep} required at {pv} (by {pby}) and {ver} (by {})", m.name));
                    continue;
                }
            }
            ...
            let Some(dm) = parse_manifest(&dir.join("decl.toml"), report) else { continue };
            if &dm.name != dep {
                report("E3013", format!("package at {} names itself {}, expected {dep}", dir.display(), dm.name));
            }
            if &dm.version != ver {
                report("E3016", format!("package {dep}: manifest version {} differs from required pin {ver}", dm.version));
            }
            packages.push(ResolvedPackage { name: dep.clone(), version: dm.version.clone(), dir: dir.clone(), hash: package_hash(&dir) });
            visit(&dm, root_dir, packages, required_by, report);
        }
    }
    visit(&manifest, &root_dir, &mut packages, &mut required_by, &mut report);
    ...
    let resolver: Resolver = {
        ...
        Rc::new(move |spec: &str, from_dir: &Path| {
            ...
            let from_pkg_dir = packages.iter().find(|p| from_abs.starts_with(&p.dir)).map(|p| p.dir.clone()).unwrap_or_else(|| root_dir.clone());
            let from_manifest = if from_pkg_dir == root_dir { Some(manifest.clone()) } else { parse_manifest(&from_pkg_dir.join("decl.toml"), &mut |_, _| {}) };
            let from_name = from_manifest.as_ref().map(|m| m.name.clone()).unwrap_or_else(|| "?".into());
            if !from_manifest.map(|m| m.has_dep(pkg)).unwrap_or(false) {
                return Err(("E3010".into(), format!("package {pkg} not declared in [dependencies] of {from_name}")));
            }
            let Some(p) = packages.iter().find(|p| p.name == pkg) else {
                return Err(("E3004".into(), format!("package {pkg} could not be resolved")));
            };
            Ok(if rest.is_empty() { p.dir.clone() } else { p.dir.join(rest) })
        })
    };
    Some(PackageUniverse { root_dir, manifest, packages, resolver, diags })
}
...
pub fn lock_text(u: &PackageUniverse) -> String {
    ...
    let lines: Vec<String> = ps.iter().map(|p| format!("{} {} {}", p.name, p.version, p.hash)).collect();
...
pub fn verify_lock(u: &PackageUniverse) -> Vec<Diag> {
    ...
    let mut out = vec![];
    let mut report = |code: &str, message: String| out.push(Diag::error(message, String::new(), Some(code)));
    ...
    for line in text.split('\n') {
        ...
        locked.push((parts[0].to_string(), parts.get(1).unwrap_or(&"").to_string(), parts.get(2).unwrap_or(&"").to_string()));
    }
    for p in &u.packages {
        ...
        if *v != p.version {
            report("E3016", format!("lock: {} version {v} differs from manifest {}", p.name, p.version));
        } else if *h != p.hash {
