...
check(`tarball built: ${filename} (${(size / 1024).toFixed(0)} KB packed, ${(unpackedSize / 1024).toFixed(0)} KB unpacked)`, existsSync(tarball));
const names = files.map(f => f.path);
check('tarball carries dist bins, wasm, README, LICENSE', ['dist/cli.js', 'dist/lsp.js', 'dist/index.js', 'dist/tree-sitter-decl.wasm', 'README.md', 'LICENSE', 'package.json'].every(f => names.includes(f)), names.join(' '));
check('tarball ships no sources or tests', !names.some(f => f.startsWith('src/') || f.startsWith('test/')));
...
writeFileSync(join(dir, 't.decl'), 'type T = { a: int, const b = a * 2 }\nexport output t: T = { a: 21 }\n');
const ev = spawnSync(bin, ['evaluate', join(dir, 't.decl'), '--root', 't'], { encoding: 'utf8' });
check('installed decl evaluates', ev.status === 0 && ev.stdout.trim() === '{"a":21,"b":42}', ev.stderr + ev.stdout);
writeFileSync(join(dir, 'bad.decl'), 'type Bad = 10..3\n');
const chk = spawnSync(bin, ['check', join(dir, 'bad.decl')], { encoding: 'utf8' });
check('installed decl checks (wasm resolved from dist/)', chk.status === 1 && chk.stderr.includes('E4011'), chk.stderr);
writeFileSync(join(dir, 'm.decl'), 'const x=1+2\n');
const fmt = spawnSync(bin, ['fmt', join(dir, 'm.decl')], { encoding: 'utf8' });
check('installed decl fmt', fmt.status === 0 && readFileSync(join(dir, 'm.decl'), 'utf8') === 'const x = 1 + 2\n', fmt.stderr);
...
const l = spawnSync(lsp, [], { encoding: 'utf8', input: frame(body) + frame(exitBody), timeout: 20000 });
check('installed decl-lsp initializes, then exits on request', l.status === 0 && l.stdout.includes('"hoverProvider":true'), `status ${l.status} ${l.stderr.slice(0, 200)}`);

// 5. the library entry
writeFileSync(join(dir, 'lib.mjs'), `
...
check('library entry importable', lib.status === 0 && lib.stdout.trim() === '{"errors":0,"checks":0,"decls":1}', lib.stderr + lib.stdout);
