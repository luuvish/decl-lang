...
export function subsumes(env: Env, a: RT, b: RT, assume: Assume = new Map()): boolean {
  ...
  if (b.t === 'pred') {
    if (a.t === 'pred') {
      return subsumes(env, a.base, b.base, assume)
  ...
  switch (b.t) {
    case 'prim':
      ...
      if (a.t === 'range') return a.base === b.name || (b.name === 'int' && a.base === 'int') || (b.name === 'float' && a.base === 'float');
    ...
    case 'range': {
      ...
      if (a.t === 'range') {
        ...
        const aHi = a.excl ? dec(a.hi) : a.hi, bHi = b.excl ? dec(b.hi) : b.hi;
        return a.lo >= b.lo && aHi <= bHi;
      }
      return false;
    }
    case 'pattern':
      if (a.t === 'lit' && typeof a.v === 'string')
        return new RegExp(`^(?:${b.src})$`).test(a.v);
    ...
    case 'arr': {
      ...
      const aLo = a.lo ?? 0, aHi = a.hi ?? Infinity;
      const bLo = b.lo ?? 0, bHi = b.hi ?? Infinity;
      return aLo >= bLo && aHi <= bHi;
    }
    case 'map':
      return a.t === 'map'
        && subsumes(env, a.key, b.key, assume)
        && subsumes(env, a.val, b.val, assume);
    case 'quantity': return a.t === 'quantity' && a.dim === b.dim;
    case 'ref': return a.t === 'ref' && subsumes(env, a.target, b.target, assume);
    case 'func': {
      if (a.t !== 'func' || a.params.length !== b.params.length) return false;
      return b.params.every((bp: RT, i: number) => subsumes(env, bp, a.params[i], assume))  // contravariant
        && subsumes(env, a.ret, b.ret, assume);                                             // covariant
    }
    case 'rec': {
      ...
      for (const m of b.members) {
        ...
        const typeOk = () =>
          mTypes.length === 0 || sTypes.length === 0 ||
          mTypes.every(mt => sTypes.some(st => subsumes(env, st, mt, assume)));
        switch (m.kind) {
          case 'req':
            if (!s || s.kind === 'opt') { set!.delete(b); return false; }
            if (!typeOk()) { set!.delete(b); return false; }
            break;
          case 'opt': case 'dflt':
            if (s && !typeOk()) { set!.delete(b); return false; }
            break;
          case 'der':
            if (!s || !typeOk()) { set!.delete(b); return false; }
            break;
...
function dec(v: any): any { return typeof v === 'bigint' ? v - 1n : v - 1; }
...
function predEq(a: any, b: any): boolean {
  ...
  if (a?.e === 'call' && b?.e === 'call') {
    return predEq(a.fn, b.fn)
      && a.args.length === b.args.length
      && a.args.every((x: any, i: number) => x.e === 'lit' && b.args[i].e === 'lit' && valEq(x.v, b.args[i].v));
  }
  return false;
}
  ...
  if (t.t === 'isectN') {
    ...
    for (let i = 0; i < arms.length; i++)
      for (let j = i + 1; j < arms.length; j++)
        if (disjoint(env, arms[i], arms[j])) return true;
