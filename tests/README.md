...
| `render/` | the renderer's data ([README](render/README.md)): `formats.json` (a golden's YAML and indented JSON forms), `inputs/` (the YAML twin of every bound document), `invalid/` (documents the reader refuses, with their messages) | `decl-ts/tests/render_test.ts`, `decl-rs/tests/render_test.rs`, `decl-py/tests/render_test.py`; the harness's `yaml-input` and `format` sections |
