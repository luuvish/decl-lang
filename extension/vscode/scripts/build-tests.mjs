...
await build({ entryPoints: [join(root, 'test/runTest.ts'), join(root, 'test/runWebTest.ts')], bundle: true, platform: 'node', format: 'cjs', target: 'node18', outdir: join(root, 'dist/test'), external: ['vscode', '@vscode/test-electron', '@vscode/test-web'] });
