...
async function stopClient() { if (client) { await client.stop(); client = undefined; } }
...
async function pickRoot(doc: vscode.TextDocument, kind: 'output' | 'input' | 'any'): Promise<string | undefined> {
  const re = kind === 'output' ? /^(?:export\s+)?output\s+([A-Za-z_][A-Za-z0-9_]*)/gm
    : kind === 'input' ? /^(?:export\s+)?input\s+([A-Za-z_][A-Za-z0-9_]*)/gm
    : /^(?:export\s+)?(?:output|input)\s+([A-Za-z_][A-Za-z0-9_]*)/gm;
  const names = [...doc.getText().matchAll(re)].map(m => m[1]);
  if (names.length === 0) { vscode.window.showInformationMessage(`Decl: the module declares no ${kind === 'any' ? 'root' : kind}`); return undefined; }
...
function pretty(compact: string): string {
  try { return JSON.stringify(JSON.parse(compact), null, 2); } catch { return compact; }
}
async function openPreview(root: string | null) {
  const doc = activeDecl();
  if (!doc) return;
  const name = root ?? await pickRoot(doc, 'output');
  const uri = vscode.Uri.parse(`${PREVIEW_SCHEME}:${path.basename(doc.fileName)}${name ? `/${name}` : ''}.json?${encodeURIComponent(doc.uri.toString())}`);
...
function refreshPreviews(source?: vscode.Uri) {
  for (const [key, src] of previews) if (!source || src.uri.toString() === source.toString()) previewEmitter.fire(vscode.Uri.parse(key));
}
...
async function bindInput() {
  ...
  if (!name) return;
  const picked = await vscode.window.showOpenDialog({ canSelectMany: false, filters: { JSON: ['json'] }, title: `bind ${name}` });
...
async function validate() {
  ...
  if (!r) return;
  const lines = [...(r.verdicts ?? []).map((v: any) => v.errors === 0 && v.warnings === 0 ? `${v.name}: ok` : `${v.name}: ${v.errors} error(s), ${v.warnings} warning(s)`)];
  ...
  (bad ? vscode.window.showWarningMessage : vscode.window.showInformationMessage)(`Decl: ${lines.join('; ') || 'no roots'}`, 'Show Output').then(pick => { if (pick) output.show(); });
}
...
async function trace() {
  ...
  const ed = vscode.window.activeTextEditor;
  if (!doc || !ed) return;
  const word = doc.getText(doc.getWordRangeAtPosition(ed.selection.active, /[A-Za-z_$][A-Za-z0-9_$.\[\]"]*/));
  const p = await vscode.window.showInputBox({ prompt: 'the canonical path to trace', value: word });
  if (!p) return;
...
function declCli(context: vscode.ExtensionContext): string[] {
  const configured = (cfg().get<string>('server.path') ?? '').trim();
  if (configured) {
    const beside = path.join(path.dirname(configured), process.platform === 'win32' ? 'decl.exe' : 'decl');
...
function openRepl(context: vscode.ExtensionContext) {
  ...
  for (const [name, file] of Object.entries(cfg().get<Record<string, string>>('inputs') ?? {})) args.push('--input', `${name}=${file}`);
  repl = vscode.window.createTerminal({ name: 'Decl REPL', shellPath: cli[0], shellArgs: args, cwd: folder?.uri.fsPath });
  repl.show();
}
function sendToRepl(context: vscode.ExtensionContext) {
  ...
  const sel = ed.selection.isEmpty ? ed.document.lineAt(ed.selection.active.line).text : ed.document.getText(ed.selection);
...
async function runFixtures(context: vscode.ExtensionContext) {
  ...
  const cli = declCli(context);
  const task = new vscode.Task({ type: 'decl', command: 'validate' }, vscode.TaskScope.Workspace, `validate ${path.basename(dir)}`, 'decl',
    new vscode.ProcessExecution(cli[0], [...cli.slice(1), 'validate', dir]), '$decl');
...
class DeclTaskProvider implements vscode.TaskProvider {
  ...
  provideTasks(): vscode.Task[] {
    ...
    const mk = (command: string, args: string[]) =>
      new vscode.Task({ type: 'decl', command, args }, vscode.TaskScope.Workspace, `${command} ${args.join(' ')}`.trim(), 'decl',
        new vscode.ProcessExecution(cli[0], [...cli.slice(1), command, ...args]), '$decl');
  ...
  resolveTask(task: vscode.Task): vscode.Task {
    ...
    const cli = declCli(this.context);
    return new vscode.Task(def, task.scope ?? vscode.TaskScope.Workspace, task.name, 'decl',
      new vscode.ProcessExecution(cli[0], [...cli.slice(1), def.command, ...(def.args ?? [])]), '$decl');
  }
}

async function selectServer(context: vscode.ExtensionContext) {
  const items: vscode.QuickPickItem[] = [{ label: 'bundled', description: 'the reference implementation shipped with the extension' }, { label: 'decl-lsp on PATH', description: 'whichever implementation is installed' }, { label: 'custom path…' }];
  ...
  else if (pick.label === 'custom path…') { value = (await vscode.window.showInputBox({ prompt: 'path to decl-lsp' })) ?? ''; if (!value) return; }
  ...
  context.subscriptions.push(output, status,
    ...
    vscode.workspace.onDidChangeTextDocument(e => {
      if (e.document.languageId === 'decl' && cfg().get('preview.refresh') === 'type') {
        ...
        typeTimer = setTimeout(() => refreshPreviews(e.document.uri), cfg().get<number>('evaluate.idleDelay') ?? 300);
      }
    }),
    vscode.workspace.onDidChangeConfiguration(async e => {
      if (e.affectsConfiguration('decl.server')) { await stopClient(); await startClient(context); }
      else if (e.affectsConfiguration('decl.inputs')) refreshPreviews();
    }),
    vscode.window.onDidCloseTerminal(t => { if (t === repl) repl = undefined; }),
  );
  await startClient(context);
}
let typeTimer: ReturnType<typeof setTimeout> | undefined;

export async function deactivate() { await stopClient(); }
