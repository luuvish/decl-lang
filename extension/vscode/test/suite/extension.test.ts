...
const fixture = (name: string) =>
  vscode.Uri.file(path.join(vscode.workspace.workspaceFolders![0].uri.fsPath, name));
...
suite('vscode-decl', () => {
  ...
  test('the server publishes diagnostics with positions', async () => {
    ...
    assert.strictEqual(codeOf(d.code), 'E4011');
  ...
  test('hover, definition, completion, and formatting come from the server', async () => {
    ...
    assert.ok(
      typeDefs.length &&
        typeDefs[0].uri.fsPath.endsWith('lib.decl') &&
        typeDefs[0].range.start.line === 0,
      'go to type definition of the output s reaches Service',
    );
