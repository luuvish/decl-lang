...
pub fn evaluate(path: &str, opts: &EvaluateOptions) -> Result<BTreeMap<String, String>, DeclError> {
    ...
    let checks: Vec<Diagnostic> = r.modules.iter().flat_map(|m| {
        ...
        check_module(&m.decls, Some(m.env.clone())).iter().map(|d| tagged(&tag, d)).collect::<Vec<_>>()
    }).collect();
    if !checks.is_empty() {
        return fail("", checks);
    }
    ...
    for n in &names {
        ...
        out.insert(n.clone(), eng.serialize(&v, n));
    }
    Ok(out)
}
...
pub fn validate(path: &str, inputs: &[(String, Document)]) -> Result<Vec<Diagnostic>, DeclError> {
    ...
    let checks: Vec<Diagnostic> = check_module(&parsed.decls, None).iter().map(|d| tagged(path, d)).collect();
    if !checks.is_empty() {
        return Ok(checks);
    }
    if !inputs.is_empty() {
