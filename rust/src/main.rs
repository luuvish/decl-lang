//! The `decl` CLI (native Rust runtime): evaluate / validate. Output is
//! byte-identical to the reference implementation's CLI so the two can
//! be diffed (python/scripts/differential.py --rust).
mod ast;
mod engine;
mod modules;
mod parse;
...
use modules::{load_modules, run_pipeline, run_universe, Bind};
use semantics::{json_str, read_json, Diag};
use std::path::Path;
...
fn evaluate(file: &str, root: Option<&str>) -> (i32, Option<String>, Vec<Diag>) {
...
fn validate_file(file: &str, input: Option<&str>) -> Vec<Diag> {
    let r = load_modules(Path::new(file));
...
fn main() {
    ...
    let Some(cmd) = args.first().cloned() else { std::process::exit(usage()) };
    ...
    let code = match cmd.as_str() {
        "evaluate" => {
            let Some(f) = pos.first() else { std::process::exit(usage()) };
        ...
        "validate" => {
            let Some(target) = pos.first() else { std::process::exit(usage()) };
