...
| PyPI | `pip install decl-lang` | A native Python runtime for `evaluate` / `validate` and a Python API. `check` / `fmt` / `decl-lsp` run the bundled reference under Node (`pip install 'decl-lang[node]'` lets pip provide Node). |
