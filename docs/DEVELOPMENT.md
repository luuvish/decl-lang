...
| `make test-typescript` | `npm test -w decl-lang`: the corpus drivers `decl-ts/tests/*_test.ts` through `node --test`, sequentially |
| `make test-rust` | `cargo test --locked --release`: the corpus drivers `decl-rs/tests/*_test.rs`, sharing `tests/common/` (`examples/api_corpus.rs` is the API corpus driver the harness runs — Cargo's place for a runnable auxiliary the crate does not ship; the reference's is `decl-ts/scripts/api-corpus.ts`, Python's `decl-py/scripts/api_corpus.py`) |
| `make test-python` | `pytest`: the corpus drivers `decl-py/tests/*_test.py`, and the API's own surface — in `decl-py/.venv`, which `make` creates with the package, its `dev` extra, and the compiled grammar |
| `make parity` | `tests/parity/differential.py`: every command line (the whole surface), REPL session, API case, and language-server session of the reference against the Rust and Python implementations, byte for byte (exit code, stdout, stderr, answers), and the goldens |
...
`modules/`, `packages/`, `subsume/`, `golden/`, `repl/`, `api/`, `lsp/`,
`fmt/`, `cli/`; `tests/README.md`) that all three run, one driver per
corpus in every language, file for file; the internal checks
(`tests/internal/checks.json`) are defined once and carried by each
suite under `tests/internal/` in its own code — a per-language test
otherwise only covers a surface that exists in that language alone.
...
| Rust (`decl-rs`, `extension/zed`) | the compiler, warnings denied | clippy, `-D warnings`, all targets; rustdoc, `-D warnings` (`missing_docs` on) | rustfmt (`max_width = 100`) | `cargo test --locked --release` (the doctests included) | `rustfmt.toml`, `[workspace.lints]` in `Cargo.toml`, `#![warn(missing_docs)]` in `lib.rs` |
| Python (`decl-py`) | mypy `strict` (`warn_return_any` off: the AST and values are `dict[str, Any]`) | ruff (`E F W I UP B SIM RUF`, line 100, py310) | ruff format | pytest (`tests/`: one corpus driver per file, the command line as a subprocess, the API directly) | `[tool.ruff]`, `[tool.mypy]`, `[tool.pytest.ini_options]` in `pyproject.toml`; `dev` extra |

### API documentation

Each package documents its public API in its language's generated
form, to one bar (roadmap Phase 8):

| Package | Generated by | Bar |
|---|---|---|
| `decl-rs` (docs.rs) | rustdoc: the crate page is `README.md` (`#![doc = include_str!(…)]`, its Rust example compiled as a doctest), its "Library layout" section the module map, every public item documented | `#![warn(missing_docs)]` under clippy's `-D warnings`; `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` in `make lint-rust`; `[package.metadata.docs.rs]` and `documentation` in `Cargo.toml` |
| `decl-py` | docstrings on the package's API (`src/decl/api.py`, `__init__.py`) and a module docstring on every module; the ports' internal functions keep the reference's shape, not docstrings | ruff `D1` on the package's API (the internal modules, tests, and scripts excepted per file); mypy `strict` as today |
| `decl-ts` | TSDoc on the entries (`index.ts`, `core.ts`, `api.ts`): every export group named and described | `tsc` (the types are the contract); read in review — no docstring linter is configured |

The three packages' READMEs, which the website syncs, describe the same
functions in the same order; the crate's README doubles as its docs.rs
page, so its Rust example is written as a function body — compiled by
`cargo test`, never run.
...
`decl-py/src/decl/api.py`, `extension/vscode/package.json`,
...
- Tests: each implementation's drivers live under its `tests/` directory,
  one file per corpus named `<corpus>_test.<ext>` (`api_test.ts`,
  `api_test.rs`, `api_test.py`), shared helpers under `tests/common/`
  (Python adds pytest's `conftest.py`); the sources sit under `src/`
  in the three — `decl-ts/src/*.ts`, `decl-rs/src/*.rs`,
  `decl-py/src/decl/*.py` — with the same module names.
