...
|---|---|---|
| `decl-rs` (docs.rs) | rustdoc: the crate page from `README.md` (`#![doc = include_str!(…)]`, so its examples run as doctests), every public item, the module map of `AGENTS.md` | `#![warn(missing_docs)]` under clippy's `-D warnings`; `cargo doc --no-deps` warning-free in `make lint-rust`; `[package.metadata.docs.rs]` in `Cargo.toml` |
| `decl-py` | docstrings on `decl/__init__.py` and the runtime's public functions, in the same words as the crate's | ruff's docstring rules (`D`) on the public API; mypy `strict` as today |
| `decl-ts` | TSDoc on the entries (`index.ts`, `core.ts`, `api.ts`) | `tsc` with `--declaration` clean; ESLint's `jsdoc` rules on exported symbols |

An item public only because a port mirrors the reference's shape is
narrowed (`pub(crate)`, a leading underscore) rather than documented;
