// Binding, evaluation, validation, serialization. Throwaway (ROADMAP §0.6).
import {
...
import type { Expr } from './syntax.ts';
...
export class Engine {
  ...
  runAsserts(inst: RecInst, asserts: any[], rootName: string) {
    ...
    for (const a of asserts) {
      ...
      if (a.tail.t === 'inline') {
        let msg = '';
        for (const p of a.tail.template) msg += typeof p === 'string' ? p : this.toStr(this.ev(reparse(p), sc));
        ...
        let msg = '';
        for (const p of d.template) msg += typeof p === 'string' ? p : this.toStr(this.ev(reparse(p), psc));
...
import { Parser, lex } from './syntax.ts';
function reparse(toks: any): Expr {
  return new Parser((toks as any[]).concat([{ kind: 'eof', nl: true, pos: 0 }])).expr();
}
