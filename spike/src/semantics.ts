// Dynamic semantics for the spike subset. Throwaway (ROADMAP §0.6).
import type { Decl, Expr, MemberAst, TypeAst, ElseTail, Tok } from './syntax.ts';
...
export class Env {
  ...
  diags = new Map<string, { params: { name: string }[]; severity: string; template: (string | Tok[])[] }>();
  registry: RecInst[] = [];
