...
.PHONY: verify parity build-typescript test-typescript build-rust test-rust python-env test-python clean
...
typescript/node_modules: typescript/package-lock.json
...
test-rust: build-rust
	rust/target/release/decl validate tests/validation
...
python-env: build-typescript
	...
	$(VPY) -m pip install -q -e ./python
	cd python && $(VPY) setup.py -q build_ext --inplace

test-python: python-env
	cd python && $(VPY) scripts/e2e.py
...
clean:
	rm -rf typescript/dist rust/target python/build python/decl/*.egg-info python/decl/_tree_sitter/*.so
