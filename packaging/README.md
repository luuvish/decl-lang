...
|---|---|---|---|
| GitHub release | `v0.4.7`: `decl` and `decl-lsp` for six platforms, the wheels, the `.vsix` | [releases/tag/v0.4.7](https://github.com/luuvish/decl-lang/releases/tag/v0.4.7) | **published 2026-09-20** by `release.yml` |
| npm | `decl-lang` | `npm install -g decl-lang` | **published 2026-09-20** (0.4.7, by `release.yml` through trusted publishing, with provenance; `decl-ts/`) |
| PyPI | `decl-lang` | `pip install decl-lang` | **published 2026-09-20** (0.4.7: 39 wheels and the sdist, by `release.yml` through trusted publishing; `decl-py/`) |
| Homebrew | tap `luuvish/tap`, formula `decl-lang` | `brew install luuvish/tap/decl-lang` | **published 2026-09-20** (0.4.7, by `release.yml`): [luuvish/homebrew-tap](https://github.com/luuvish/homebrew-tap), the formula mirrored from `homebrew/` |
| crates.io | `decl-lang` (bins `decl`, `decl-lsp`) | `cargo install decl-lang` | **published 2026-09-20** (0.4.7, by `release.yml` through trusted publishing; `decl-rs/`) |
| Visual Studio Marketplace, Open VSX | `luuvish.vscode-decl` (the VS Code extension, bundling npm `decl-lang`) | Extensions view: "Decl" | **published 2026-09-20** on both (`luuvish.vscode-decl` 0.4.7, by `release.yml` with `VSCE_PAT` / `OVSX_PAT`); the Open VSX namespace claim is filed |
| Zed extension registry | `decl` (the Zed extension: grammar, queries, `decl-lsp` pointer) | Zed: extensions, "Decl" | submitted 2026-09-05, awaiting review: [zed-industries/extensions#7488](https://github.com/zed-industries/extensions/pull/7488), updated to 0.4.7 on 2026-09-20 (the submodule at the `v0.4.7` tag, the extension under `extension/zed`; the `decl-lsp` binaries come from the GitHub release). Until it is merged the registry has no entry to bump, so a new version updates this pull request; after that, a version is a pull request of its own |

0.4.7 followed 0.4.6 within the day, for Windows. The 0.4.6 tag's run
published npm, crates.io, Homebrew, and the two extension stores, and
stopped before PyPI and the GitHub release: the Windows wheels failed their
`decl --version`, because the Python command line asked for a thread stack
CPython refuses on Windows. The Python files of 0.4.6 are therefore the tag
plus that one fix (`85e7d29`), built and published by a dispatch of
`release.yml`, and its GitHub release was put together by hand from the two
runs' artifacts. `verify.yml` has run the installed Python package on Windows
since; its first run there found that the package wrote carriage returns on
Windows. 0.4.7 carries both fixes, and every channel of it was published by
the tag's own run.
