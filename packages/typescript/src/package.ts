...
// Implementation conventions (documented in packages/typescript/README.md): dependency
...
export function parseManifest(path: string, report: (c: string, m: string) => void): Manifest | null {
  ...
  for (const line0 of src.split('\n')) {
    ...
    if (sec) {
      section = sec[1];
      if (section !== 'dependencies') { report('E3011', `manifest ${path}: unknown section [${section}] (fail-closed, D28)`); ok = false; }
      continue;
    }
    const kv = /^([A-Za-z0-9_-]+)\s*=\s*(.+)$/.exec(line);
    if (!kv) { report('E3011', `manifest ${path}: unparseable line "${line}"`); ok = false; continue; }
    const key = kv[1];
    const raw = kv[2].trim();
    const value = raw.startsWith('"') ? JSON.parse(raw.replace(/\\/g, '\\\\')) : raw;
    if (section === 'dependencies') {
      if (!NAME_RE.test(key)) { report('E3013', `manifest ${path}: invalid package name ${key}`); ok = false; continue; }
      if (typeof value !== 'string' || !VERSION_RE.test(value)) {
        report('E3012', `manifest ${path}: dependency ${key} = ${raw} is not an exact semantic-version pin`); ok = false; continue;
      }
      deps.set(key, value);
    } else if (section === null) {
      if (!SEMANTIC.includes(key) && !METADATA.includes(key)) {
        report('E3011', `manifest ${path}: unknown field ${key} (fail-closed, D28)`); ok = false; continue;
      }
  ...
  if (!NAME_RE.test(name)) { report('E3013', `manifest ${path}: invalid package name ${JSON.stringify(name)}`); ok = false; }
  if (!VERSION_RE.test(version)) { report('E3012', `manifest ${path}: version ${JSON.stringify(version)} is not an exact triple`); ok = false; }
...
export function openPackageUniverse(entryFile: string): PackageUniverse | null {
  ...
  if (!manifest) return { rootDir, manifest: { name: '?', version: '0.0.0', dependencies: new Map() }, packages: new Map(), resolver: () => ({ code: 'E3011', message: 'unusable manifest' }), diags };
  ...
  const visit = (m: Manifest, byDir: string) => {
    for (const [dep, ver] of m.dependencies) {
      const prev = requiredBy.get(dep);
      if (prev && prev.version !== ver) {
        report('E3014', `package ${dep} required at ${prev.version} (by ${prev.by}) and ${ver} (by ${m.name})`);
        continue;
      }
      ...
      if (!dm) continue;
      if (dm.name !== dep) report('E3013', `package at ${dir} names itself ${dm.name}, expected ${dep}`);
      if (dm.version !== ver) report('E3016', `package ${dep}: manifest version ${dm.version} differs from required pin ${ver}`);
      packages.set(dep, { name: dep, version: dm.version, dir, hash: packageHash(dir) });
      visit(dm, dir);
    }
  };
  visit(manifest, rootDir);
  ...
  const resolver: PackageResolver = (spec, fromDir) => {
    ...
    const fromPkgDir = [...packages.values()].find(p => absPath(fromDir).startsWith(p.dir))?.dir ?? rootDir;
    const fromManifest = fromPkgDir === rootDir ? manifest : parseManifest(join(fromPkgDir, 'decl.toml'), () => { })!;
    if (!fromManifest.dependencies.has(pkg))
      return { code: 'E3010', message: `package ${pkg} not declared in [dependencies] of ${fromManifest.name}` };
...
export function verifyLock(u: PackageUniverse): Diag[] {
  ...
  for (const p of u.packages.values()) {
    ...
    if (!l) { report('E3015', `lock: missing entry for ${p.name}`); continue; }
    if (l.version !== p.version) report('E3016', `lock: ${p.name} version ${l.version} differs from manifest ${p.version}`);
