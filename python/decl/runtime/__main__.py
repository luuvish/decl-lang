...
Output formats match the reference `decl` CLI byte for byte so the two
can be diffed (see impl/test/differential.ts)."""
...
from .modules import load_modules, run_universe
...
def judge_fixture(file: str, is_valid: bool) -> dict:
    ...
    with open(file, encoding="utf-8") as f:
        src = f.read()
    meta = dict(m for m in __import__("re").findall(r"// @([a-z-]+):\s*(.+)", src))
