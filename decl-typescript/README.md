...
client at `node decl-typescript/src/lsp.ts` for `.decl` files, e.g. in VS Code
via a generic LSP client extension, or in Neovim:

```lua
vim.lsp.start({ name = 'decl', cmd = { 'node', '<repo>/decl-typescript/src/lsp.ts' }, filetypes = { 'decl' } })
```
